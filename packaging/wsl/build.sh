...
inputs="$repo/packaging/inputs/wsl-$arch"
python3 "$repo/scripts/validate_package_inputs.py" --root "$inputs" --source-root "$repo" --version "$version" --target "$target" \
...
cp -R -- "$inputs/legal/." "$stage/legal/"
cp -R -- "$inputs/sbom/." "$stage/sbom/"
cp -- "$inputs/README-OFFLINE.txt" "$stage/README-OFFLINE.txt"
cp -- "$inputs/package-input-inventory.json" "$inputs/package-input-review.json" "$stage/"
