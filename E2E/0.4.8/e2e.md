...
| E: provider onboarding | **pass**, 7 of 7 | not run | **driven, evidence not retained** | not run | two real credentials, a live 28 model catalog and a live 10 model catalog, one bounded readiness call each, and the secret absent from the database, WAL, SHM and evidence. **Windows**: these cells were driven earlier in the same session, including the browser authorization the owner approved, but the per-cell captures lived in a transcript that was later compacted, so they cannot be re-read and are not marked from re-readable observation. What does survive on disk is a credential file written at `16:56`, a Gemini provider the daemon reports connected and default, and seven runs that completed against that provider, none of which could exist had onboarding not worked. **That is corroboration, not the cell evidence the runbook asks for, so the row is owed a re-run rather than a mark** |
...
| G: pair and the QR | **pass**, 4 of 4 | not run | **pass**, 3 of 4, `G3` not run | not run | `G3` decodes the printed symbol with an independent decoder; `G4` is a real handset, and the daemon recorded the claim at `200` and the device row |
...
| **overall** | **pass**, 1 partial | not run | **incomplete**: 36 of 45 cells | not run | **Windows**: A, B, C, D, F and H were driven to their stated oracles and pass; G is 3 of 4; E is owed a re-run for evidence; `G3` and `H4` are named below. One product defect was found and fixed on this OS (`F7`), and one earlier finding was **retracted** (`F9`). **The count is 36 because 9 cells are genuinely not closed here, not because the pass was called done at a convenient number.** | every part of this runbook has been driven on WSL and every cell has a verdict. The one row that is not a clean pass is named above and it is not a WSL defect: `H4` has no public path to it. Three defects were found and fixed during the pass, each with a regression test seen red first |
