...
| C13 | the transport is `tailscale`, and the **transport address only** holds the port: `harness/hold_port.py` is not enough here, so the cell binds `<tailnet-ip>:<port>` and leaves `127.0.0.1:<port>` free | `vadgr start` | it walks up rather than starting: the daemon binds every host it advertises, so a port free on loopback and taken on the tailnet is not usable. The port file names the port it took and health answers on **both** hosts | CLI output, the two bind attempts, the port file, `curl` health on loopback and on the tailnet address | stop, release the held address | `not run` |
...
| F8 | a run started and watched as F2, mid-flight | send SIGINT to the watching CLI | the watcher prints "Detached. The run continues." and **exits `130`**; the run was **not** cancelled: the API row later reads `completed` | the watcher's output and exit code; the row after | none | **fail on native Windows**, in one half of the cell. **What passed**: the run was not cancelled by the interrupt and later read `completed`. **What failed**: the watcher printed no `Detached. The run continues.` and exited `3221225786`, which is `STATUS_CONTROL_C_EXIT`, not `130`. The cause is in the code rather than the harness: `tokio::signal::ctrl_c()` appears only in the logs follower at `src/cli/commands/service.rs:616`, so the run watcher installs no handler and the operating system default terminates it. `CliError::Detached` and its exit `130` exist in `src/cli/error.rs` but nothing on this path reaches them. On a Unix host an unhandled `SIGINT` yields `130` by shell convention, which would report the right code while the message was still never printed. |
