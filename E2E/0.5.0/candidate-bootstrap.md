...
Tested source: `5cd8065676cfd9faab674ff1a5a2c49718a97500`.
...
|---|---|---|---|---|---|
| B01-WSL | WSL Python, no signing variables; record commit and script hashes | Invoke the public preflight command with a non-default workflow ref and dummy SHA | Nonzero refusal; no materialized source | Filesystem listing shows no output; API is not contacted | pass: repaired-pass.md; exact command, exit and filesystem oracle |
| B01-Windows | Native Windows Python; same setup as B01-WSL | Invoke the same preflight command through Windows Python | Same refusal and no materialized source | Native process exit and Windows filesystem listing | pass: repaired-pass.md; exact command, exit and filesystem oracle |
| B02-traversal-WSL | New traversal ZIP fixture on WSL; dummy authorization binds this commit | Invoke `extract-verified` for this fixture | Refusal names the unsafe archive property before extraction | No extraction directory or outside file exists; fixture SHA256 unchanged | pass: repaired-pass.md; exact command, exit and filesystem oracle |
...
| B03-WSL | A one-member authorization ZIP and a second ZIP with an extra member | Extract each through `scripts/candidate/record.py` | Valid record extracted exactly; extra member refused | Compare source/extracted SHA256 and absent second output | pass: repaired-pass.md; exact command, exit and filesystem oracle |
| B03-Windows | Native Windows paths, same two record cases | Run the record command with Windows Python | Same exact-copy success and extra-member refusal | Native SHA256 and process exit | pass: repaired-pass.md; exact command, exit and filesystem oracle |
| B04-Windows | Production PowerShell script and dummy non-default ref | Invoke signer with missing input and no vendor variables | Refuse before vendor access or file mutation | Native exit, script output, unchanged isolated directory | pass: repaired-pass.md; exact command, exit and filesystem oracle |
| B05-Windows | Production PowerShell script, attempt two, default-branch context | Invoke signer with missing input and no vendor variables | Refuse the rerun before reading a signing ledger | Native exit and absent ledger | pass: repaired-pass.md; exact command, exit and filesystem oracle |
