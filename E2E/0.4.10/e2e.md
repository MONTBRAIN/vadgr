...
| overall | not run: no live cell has run | **pass**: every cell and all three independent closing passes pass | **partial**, and the weakest driven part sets it: `M` is partial, so this is not a pass for the operating system. Every part this host could drive passes, and all three independent closing passes pass against one frozen head. `T1` is blocked on a second network and `M3` to `M7` wait on the owner and the handset | **pass**: every cell and all three independent closing passes pass |
