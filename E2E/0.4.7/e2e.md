...
> rerun. F17 records an invalid monitor attempt. F19 blocks bounded CUA-call
> cancellation because its child process outlived the cancelled run. Untouched
> cells remain open below.**
...
| C22 | Long cua call recorded `in_flight` and cancellable | Cancel through API/CLI while the child call is open | Call and run cancel promptly; no terminal `done` appears after cancellation; child cleanup is bounded | Timing marker, process tree, journal, DB, sockets | Remove reversible effect; stop child if owned | **fail on `21f6078`**: terminal `vadgr runs cancel` made the run `cancelled` with no `done`, but its real `sleep 30` CUA child remained alive after five seconds. F19 owns the repair and rerun. |
...
| F17 | The WSL/Windows dogfood attempt reached 27 responses although its monitor reported zero. | The monitor matched a spaced JSON form while the live JSONL used compact `"phase":"response"`; a second ad-hoc monitor was also not a valid pre-acceptance guard. | The 27-response attempt is invalid evidence. Every future bounded cell uses a pre-armed, JSON-aware monitor whose recorded count is checked against the journal before verdict. | open: the dogfood cell must rerun after F19, with a verified monitor. |
| F19 | Cancelling a live CUA shell call ended the Vadgr run but did not terminate the shell child. | Cancellation stopped the Rust dispatch future and later closed the CUA server, but the CUA child process was not in a cancellation-aware process group. | `0.4.7` must make cancellation terminate the owned CUA process tree on Linux, WSL, macOS and Windows, add a focused regression, then rerun C22 and the owner dogfood batch on every affected OS. | open: WSL C22 failed with an owned `sleep 30` process still live after five seconds; the test killed only that owned child during cleanup. |
