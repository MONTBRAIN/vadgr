...
> **Status: E2E now run on all four supported operating systems, 2026-08-17.**
> macOS was the last one owed and is driven end to end here: the gate locally,
> the whole surface sweep, all 29 `A` cells including a live ChatGPT OAuth
> approval, all 8 `BM` credential cells, 21 of 25 `C` cells, the full `D`
> hard-kill sequence with both sockets captured, 4 of 5 `E` cells and `OS-M`
> against TextEdit. **It found and fixed four defects**, two of them serious.
> `F36`: on macOS the daemon could not see the Tailscale application at all, so
> the entire phone pairing path was unreachable and the log blamed the owner's
> Tailscale state for it. `F37`: the approval gate failed open, and a live model
> deleted a file the goal had explicitly gated because it wrote a sentence in
> the `risk` field instead of a severity. `F35`: the installed cua `fs` tool
> never expanded `~`, and its own read-back confirmed the write, so a model
> reported success while the file sat in a directory named `~` inside the
> checkout. `F38`: the structured tier told macOS callers to install a Linux
> package. `E03` is the one partial, and `BM05` owes one sub-control that needs
> root.
>
> **Status of the earlier passes: E2E run on WSL and native Windows, and
> partially run on native Linux, 2026-08-17.** Native Linux drove every part of this runbook: the
...
Run on 2026-08-17 on a native macOS desktop: macOS 26.5.2 build 25F84 on arm64,
not a virtual machine. The host had no Rust toolchain and no Python 3.12; both
were installed before the first cell. The driving CLI was Claude Code 2.1.234,
detected rather than assumed.

| artifact | identity |
|---|---|
| tested head | `d4667f2`, which is `efcce05` plus the three fixes this pass made |
| release daemon | built from that head, SHA-256 `9edfa5d9af26573f1611171a0322ded948ac2a75c53efa96820ca2fae5d06b12` |
| installed cua | **`0.7.3`**, built from the fix this pass made for F35 and F38, installed non-editable outside the checkout, `vadgr-cua doctor` reports all 33 tools |
| `vadgr` entry point | an installer-shaped home first on `PATH`, resolving to `/Users/<owner>/.forge-e2e/bin/vadgr` |
| capture and input backends | `mss` capture and Quartz `CGEvent` input, with Accessibility and Screen Recording granted. The structured AT-SPI tier is not available on this platform, see F38 |

The automated gate ran locally on this host rather than in CI: engine 122, api
432, cli 152 and rust 210 passed with one Docker-only test ignored, and
`cargo fmt --check`, `cargo check --all-targets` and
`cargo clippy --all-targets -- -D warnings` each exited `0`. **The api suite did
not pass here at first.** Two tests asserted platform behaviour macOS does not
provide, and neither had ever run on a Mac because CI runs `engine`, `api` and
`cli` on Ubuntu only. `test_serve` bound `127.0.0.2`, which Linux assigns to
loopback and macOS does not, and `test_transport` bound an `AF_UNIX` socket
under `tmp_path`, which is 126 bytes on macOS against a 104-byte limit. Both are
harness defects rather than product defects: the launcher never binds
`127.0.0.2` in production, and the real tailscaled path is 35 bytes. Both are
repaired and the suite is green.

Every product command in this pass was invoked on its own and its output read
before the next was chosen, per the rule at the top of this file.

| group | macOS result |
|---|---|
| `A01`-`A06` ChatGPT OAuth | pass. One owner browser approval completed the flow; the account-scoped catalog returned exactly seven models, the committed record is an opaque `cred_v1_` `0600` file whose tokens are absent from the database, WAL and SHM, the connection survived a restart byte-identically, and `run-b0b31ac3ec3444b8b50419150ef17103` completed on `gpt-5.6-luna` in 7 responses for 40,931 input and 386 output tokens with an exact read-back |
| `A07`-`A12` OpenAI Platform key | pass. 51 live models; `run-e0ba6997312d499b90484891e6acfdfc`, 8 responses, 47,390 input and 488 output tokens |
| `A13`-`A18` Gemini key | pass. 28 live models; `run-f121a146f0824ecaad4a2324f807c614`, 11 responses, 89,289 input and 415 output tokens. Five of its ten tool calls failed and the model recovered from every one: `nproc` and `lscpu` do not exist on macOS, `/home/user` is a Linux path, and the shell guard refused a string carrying `;` while naming the fix. Each failure arrived as data on its own sequence, which is the negative path behaving correctly under a real model |
| `A19`-`A24` Anthropic key | pass **after F35 was fixed**. 10 live models. The first attempt found the defect: the model wrote its answer to a `~/` path, the tool reported success, its own read-back confirmed it, and the file was not there. `run-e85f092926254d808d157cb4ed57033a` re-run against cua `0.7.3` resolves to the real home path |
| `A25`-`A29` additive group | pass. OAuth and Gemini coexisted as two distinct records with complete catalogs, the CLI printed `Default remains: OpenAI / gpt-5.6-sol` on the Gemini connect, `run-c466296c6fa04ceb9259d16ba566a044` ran explicitly on `gemini-3.5-flash-lite` while OpenAI stayed default with a read-back matching `df -h` exactly, moving the default to Gemini left both catalogs intact, and deleting OpenAI removed exactly one record |
| `BM01`-`BM08` | pass, 8 of 8, recorded in the Part B table. One `BM05` sub-control is owed for want of `sudo` |
| `OS-M` | pass, recorded in the installed-product table |
| `D01`-`D07` | pass, 7 of 7, with both sockets captured across the kill. **`D06` is demonstrated here**, which native Windows could not do: the first post-restart call is a live-state read of the process table, the model reports that the killed command is not running, and only then retries it |
| `C01`-`C25` | 21 pass, 4 partial. `C07` to `C09` park correctly and their continuation needs the reply surface that belongs to `0.6.0`. `C21` is partial for `F34`, exactly as on Linux and Windows: the raw socket carries the cancel terminal and the phone stream cannot |
| `E01`-`E05` | 4 pass, 1 partial. `E03` is partial: the kill must land while a cua call is open, and on macOS every pixel-tier call completes in under a second, so three attempts landed between calls or after the run had finished. The same machinery is proved in `D01`, which killed with `computer-use__shell` open and both sockets recorded closing at 1006 |
| surface coverage | 47 of 47 shipped HTTP rows, all 30 absence probes, **7 of 7** callback rows and 25 of 25 CLI rows pass |

The pairing chain passes here rather than being blocked, because F36 was found
and fixed during this pass. `H18` also answered `503` at first, and the cause
was this pass leaking a daemon of its own that still held callback port `1455`,
not anything in the product. Both are recorded because both first looked like
product failures.

`CB04` is closed here independently: the live browser approval produced
`callback{method=GET path=/auth/callback}: status=303` followed by
`callback{method=GET path=/auth/complete}: status=200`, and a scan of the whole
daemon log for `code=`, `state=` or `access_token` returned zero matches, as did
a scan for any `?` character at all.

For all four credential paths the key never appeared in the CLI transcript or in
any process argument, captured as the full `argv` of the running CLI during each
login; the committed record's filename matched the opaque `cred_v1_<32 hex>`
form; a scan of the database, WAL and SHM for the exact key value returned zero
matches; and the connection, its catalog and the default survived a restart of
the installed daemon with the record's `sha256` and its `600`/`700` controls
unchanged.

**A method deviation, recorded rather than hidden.** The `A` group ceiling is 6
engine iterations. The runs took 7, 8, 11, 4 and 8. No public surface exposes a
per-run iteration cap, so enforcing that ceiling needs the driver to cancel
...
| B: credential storage | 4 platforms x 8 assertions | 32 | 32 | 0 | 0 |
| OS: installed product | 4 operating systems x 1 full live composition | 4 | 4 | 0 | 0 |
...
| Findings | corrections recorded during the pass | 38 | 31 | 1 | 6 |
| | | **274** | **164** | **2** | **108** |

Across the whole runbook the verdicts are 156 `pass`, 8 `partial`, 0 `not run`
and 2 `blocked`. **No supported operating system reads `not run` any more.** F27 and F29 were both found as Windows failures and are now
...
| BM01 | macOS | 01 | pass on `d4667f2`: the database was absent before start. The daemon log orders the assertion rather than assuming it: the recovery scan and the listener are logged at `00:30:51.629` and `.630`, and health answered only after them. `user_version` reached 1 with `devices`, `machine_settings`, `provider_catalogs`, `provider_connections`, `provider_models` and `runs`. `machine_settings` held exactly one row with `default_provider` and `default_model` both null, which is the null singleton default. All three providers reported disconnected with empty catalogs, health reported `platform: macos`, and the installed `vadgr health` agreed at exit `0`. |
| BM02 | macOS | 02 | pass on `d4667f2`: the fixture is a real `0.4.6` database produced by the `0.4.6` release binary built from tag `v0.4.6` on this host, which reported `version: 0.4.6` and created `user_version` 0 with only `devices` and `runs`. That daemon wrote a genuine historical run `run-3ca387648a5d44809d6da16f9ef925df` through `POST /api/runs`, which reached the terminal `failed` state through the legacy provider path. The database was copied through the SQLite backup API, `sha256 4faced64`, carrying one run row. The installed `0.4.7` daemon migrated it to `user_version` 1 and added the four new tables. The historical run stayed readable on both public surfaces: `GET /api/runs/<id>` returned `200` with its exact title and status, and `vadgr runs list` printed it at exit `0`. No legacy credential was imported: the credential root held zero files at mode `0700`, `provider_connections` held zero rows, and all three providers reported `connected: false`. |
| BM03 | macOS | 03 | pass on `d4667f2`: a local stand-in provider served all three catalog and completion routes through the daemon's documented endpoint configuration, and the public `vadgr provider login` connected OpenAI, Gemini and Anthropic with three unique sentinels, each at exit `0`. OpenAI needed `--auth api-key` because it is the only provider offering two methods. All three coexisted, each holding a distinct opaque `cred_v1_<32 hex>.json` reference, each file `0600` inside a `0700` directory. Resolution returned the exact sentinel: the stand-in received the matching `sha256` for each provider on both its catalog and its completion route. Rotating OpenAI moved only that reference, from `95ae426b` to `63f11606`, deleted the old file and left the Gemini and Anthropic references untouched. The public `vadgr provider logout gemini` exited `0` and removed only Gemini's connection, record and catalog. The database, WAL and SHM held zero matches for all four sentinels at every stage. |
| BM04 | macOS | 04 | pass on `d4667f2`: every committed record is a strict version 1 JSON object with exactly the allowed keys. An API-key record carries `version`, `provider`, `kind: api_key` and `api_key`; the OAuth record committed in `A04` carries `version`, `provider`, `kind: oauth`, `access_token`, `refresh_token` and `expires_at`. Each filename matches the opaque `cred_v1_<32 hex>.json` form, each reference stored in `provider_connections` resolves to a real file, and every record is a regular file at mode `600` with one hard link, owned by the running user, inside a `700` directory. `ls -le` shows no extended ACL entry and `xattr` reports zero extended attributes, which are the macOS counterparts of the `getfacl` and `getfattr` checks. No secret value was printed at any point; presence was asserted by a boolean and identity by a `sha256`. |
| BM05 | macOS | 05 | pass on `d4667f2`, with one sub-control owed. Five controls were driven, each on its own isolated copy with exactly one weakening, and every one failed closed by name. The positive control refreshed at `200`. A credential record at `0644` and at `0640` each produced `credential path mode must be 600`. A credential directory at `0755` and at `0750` each stopped the daemon before it served at all, with `credential path mode must be 700`. An extended ACL added with `chmod +a "everyone allow read"` while the mode bits were restored to `0600` produced `credential path has an extended access ACL`, which is the ACL check firing on its own rather than the mode check catching it, and it shows the check recognises a macOS ACL rather than only a POSIX one. Every refusal arrived as `PROVIDER_UNAVAILABLE` with category `credential_store_failed`. **The wrong-owner control is owed**: it needs `chown` to another uid, this host has no non-interactive `sudo`, and the same sub-control is owed on Windows for the same class of reason. |
| BM06 | macOS | 06 | pass on `d4667f2`: nine unsafe fixtures were each staged on an isolated copy and driven through the public refresh, and every one failed closed with its own name, while the valid control resolved at `200`. Malformed record JSON gave a parse position; a malformed reference gave `credential reference is malformed`; a 70 KB record gave `credential record exceeds 64 KiB`; a record naming `gemini` under the OpenAI connection gave `credential provider does not match its connection`; `version: 2` gave `unsupported credential record version 2`; an added field gave ``unknown field `extra` `` listing the three it accepts; a record replaced by a symlink gave `credential path has an unsafe type`; a credential directory replaced by a symlink stopped the daemon with `credential directory is not a regular directory`. The macOS specific case is the filesystem that cannot enforce a mode, which is this platform's counterpart of the Linux FAT16 loopback and the WSL drvfs rows: an isolated credential root was placed on a FAT16 disk image created with `hdiutil`, where `chmod 600` is silently a no-op and the file stays `-rwx------`, and the daemon refused with `credential path mode must be 600`. The image was detached and removed afterwards. |
| BM07 | macOS | 07 | pass on `d4667f2`: a staged temporary record named `.cred_stage_<32 hex>.tmp` was left beside two committed records, with the database still naming only the two committed references, which is the shape a fault before the SQLite commit leaves behind. On restart the daemon removed the staged orphan and kept both committed records: the directory listing afterwards held exactly the two `cred_v1_` files, and the public refresh resolved at `200` with both providers connected. |
| BM08 | macOS | 08 | pass on `d4667f2`: an extra committed-looking record `cred_v1_cdcd...cd.json` was left in the directory while the database named only the two current references, which is the shape a fault after the SQLite commit leaves behind. On restart the daemon removed the unreferenced record and kept both named ones, and the public refresh resolved at `200`, so the committed reference survived and still resolved. |
...
| OS-M | macOS desktop; installed release and cua; fresh state; one owner-supplied provider credential; TextEdit is available | Inspect macOS/session. Open TextEdit, enter the fixed scratch text in an unsaved document, inspect it through the editor UI, restart Vadgr and confirm provider persistence | Health says macOS; live provider and installed cua complete; journal/API/CLI/sockets and editor UI read-back agree; local Application Support controls survive | Same artifacts as OS-L plus macOS ACL/owner metadata | Close only the test unsaved document without saving; remove isolated state | pass on `d4667f2`: `run-fa896a79d2344e2d985cebe2b39dadef` completed on `gpt-5.6-luna` in 20 model responses using 167,710 input and 1,992 output tokens. The host is macOS 26.5.2 build 25F84 on arm64. Health reported `platform: macos` with `computer_use: true`, and `GET /api/computer-use/status` returned `available: true` with `platform: native` before the run was submitted. The catalog was authenticated and live at 51 models and contained the exact selected id. **The journal records a pixel-tier sequence, not a structured one**, because the structured native tier is AT-SPI and AT-SPI is Linux only: `apps`, `ui_windows`, `get_platform_info`, `get_screen_size`, `screenshot`, `key_press cmd+n`, `type_text` of exactly `Vadgr dogfood\nVerified through editor UI`, then `screenshot_region` as the read-back. See F38. No filesystem tool was called at all, so nothing was opened, created or saved, and the document stayed unsaved. Independent oracle, captured outside both vadgr and cua: TextEdit's own document text read through AppleScript holds exactly `Vadgr dogfood` and `Verified through editor UI`. After stopping only the assigned pid and restarting the same state, OpenAI stayed connected and default, the providers payload was byte-identical, the credential record's `sha256` and its `600`/`700` controls were unchanged, and the run row still served `200`. The unsaved documents were closed without saving and the editor's restorable state was cleared at cleanup. |
...
**Part C on native Windows: 25 of 25, with `C21` partial.** `C21` and `C22` were
re-run on `4135672` after `F34`, with both sockets captured this time, and the
result matches native Linux exactly: `/api/ws/runs/{id}` carries `agent_cancelled`
and `run_cancelled`, while `/api/runs/{id}/stream` stays silent because
`run_cancelled` and `agent_cancelled` sit in `NOT_YET_ON_THIS_STREAM`. `C22`
passes and closes F19's owed Windows rerun: no terminal arrived for the open call
and no cua or shell descendant survived the cancel. Each cell ran on its own state, port and
...
| C07 | parked durably at `awaiting_approval` with one `await_user` entry. **Re-run on WSL after `F37`, and `Not-Needed` on native Linux and native Windows**, with the reason written down rather than assumed. `F37` changed a JSON schema sent to the model and a Rust `match` on a lowercased string, and the only `cfg` in `policy.rs` and the whole `control/` path is `#[cfg(test)]`, so there is no platform branch to differ. Nothing sends the operating system into the model's context. The observable is the park, which happens before the gated action, so no shell, file, process or access control is touched on any host: the WSL journal holds exactly one call, `control__request_approval` at sequence 0. The fail-closed behaviour is covered by unit tests that the gate runs on all three operating systems. The residual uncertainty is that a model could send `medium` instead of `high` on some run, and that is model non-determinism rather than platform behaviour: three hosts would be three samples of one distribution **WSL re-run on `108baca` after `F37`.** `run-e6d0b8c08132444a8185736d9ca25a41` parked at `awaiting_approval` with exactly one `await_user` entry, and the gated effect had not happened: the file the goal named did not exist while the run was parked, and still did not exist after the run was cancelled. Under the new schema the model sent `risk: "high"` with an action and a preview of the exact command, and the gate recorded `high-risk action` as its reason, so the value the model now chooses still reaches the parking path. The cell stays `partial` on WSL for the reason it is partial everywhere: no answer-and-continue surface ships in `0.4.7`. |
...
| C22 | **pass on `4135672`, re-run for `F34` with both sockets captured**, using installed cua `0.7.2`, the same wheel Linux used. The precondition was observed: `computer-use__shell` was recorded `in_flight` at sequence 0 with no terminal, running a 300 second sleep. `vadgr runs cancel` returned `0` in under a second and the row reached `cancelled` with `Run was cancelled`. **No terminal `done` or `error` ever appeared for the open call**, and six seconds later the daemon had no cua child and no shell descendant left, so child cleanup is bounded. That is F19's Windows rerun, which had been owed. The sockets show the same split as `C21`: `/api/ws/runs/{id}` carried `agent_cancelled` then `run_cancelled`, `/api/runs/{id}/stream` stayed silent **WSL re-run on `dfe0f97` for `F34`, using installed cua `0.7.2` built from its tag, `sha256 3179a6602d8fc88f`, and installed without editable mode outside the checkout.** The precondition was observed: `computer-use__shell` was recorded `in_flight` at sequence 0 with no terminal, running a 300 second sleep, and the daemon's own process tree held one cua child, its `sh` and its `sleep`. `vadgr runs cancel` exited `0` in 0.11 s and the row reached `cancelled` with `Run was cancelled`. **No terminal `done` or `error` ever appeared for the cancelled call.** Walking the daemon's process tree, zero shell descendants and zero cua descendants remained within one second, so child cleanup is bounded. The sockets show the same split as `C21`: the mobile route carried `agent_cancelled` then `run_cancelled`, and the raw route stayed silent after `started` and `tool_call`. |
...
| C21 | **partial on `4135672`, re-run for `F34` with both sockets captured**, and the earlier Windows row is withdrawn for the same reason the Linux one was: it recorded an exit code and a status and never captured the sockets its own oracle asks for. The precondition was observed rather than assumed: with the stand-in holding the provider for 45 seconds, the row was `running` and the journal was empty, so a model request was open and no cua call was. `vadgr runs cancel` exited `0`, `vadgr runs get` reported `cancelled`, and the row still read `cancelled` with `Run was cancelled` after the 45 second wait had elapsed, so no later completion overwrote it. `/api/ws/runs/{id}` carried `agent_cancelled` then `run_cancelled`. `/api/runs/{id}/stream` carried `started` and `tool_call` and stayed silent, which is `NOT_YET_ON_THIS_STREAM` behaving exactly as written. Windows therefore matches Linux, and the cell stays `partial` for the same reason: one socket says cancelled, the published one cannot until its frozen vocabulary gains a member **WSL re-run on `dfe0f97` for `F34`, and it matches both other platforms.** The precondition was observed rather than assumed: with the stand-in holding the provider for 45 seconds the row read `running`, the journal held zero entries and no cua call was open. `vadgr runs cancel` exited `0` in 0.27 s and the row reached `cancelled`. `/api/ws/runs/{id}` carried `run_started`, `agent_started`, then `agent_cancelled` and `run_cancelled`, which are the frames `F34` added. `/api/runs/{id}/stream` carried `started` and `tool_call` and then stayed silent. After the full 45 second hold had elapsed the row still read `cancelled` with `Run was cancelled`, so no later completion overwrote it. The cell stays `partial` on WSL for the same structural reason as elsewhere: the published stream's vocabulary has no cancelled member yet. |
...
| F35 | The installed cua `fs` tool did not expand a leading `~`, and its own read-back hid it. A live Anthropic run on macOS was asked to write a hostname to `~/vadgr-e2e-a24.txt`. `fs write` reported `{"path": "~/vadgr-e2e-a24.txt", "written": 25}`, `fs read` of the same string returned the content, and the model reported the task done and verified. The file was not in the home directory at all. | Every `fs` operation built a `Path` straight from the caller's string, and `Path` treats a tilde as an ordinary directory name. `write` also calls `parent.mkdir(parents=True)`, so it created a directory literally named `~` beside the daemon's working directory, which here was the vadgr checkout itself: `/Users/<owner>/Code/vadgr/~/vadgr-e2e-a24.txt`, untracked, where a `git add -A` would have committed it. Read and write agreed because both resolved the same wrong path, so the round trip could never expose it. `shell.run` had the same gap in `cwd`. | `fs` and `shell` expand a leading `~` once at their dispatch boundary, so no operation can miss it. Three tests cover it, and the filesystem one asserts the real location and the absence of a `~` directory rather than the round trip, because the round trip passed while the bug was live. | pass: shipped as cua `0.7.3`. A24 was re-run against a wheel built from the fix and installed non-editable outside the checkout: the model still sends `~/vadgr-e2e-a24.txt`, the tool now reports the resolved `/Users/<owner>/vadgr-e2e-a24.txt`, the file exists there, and no `~` directory is created. |
| F36 | On macOS the daemon could not see Tailscale at all. Health reported `available: false`, every pairing route answered `TRANSPORT_UNREACHABLE`, and the log said `tailscaled not running or logged out` while the tailnet was up and this host was reachable at its 100.x address. The phone path, which is what this minor's transport exists for, was unreachable on macOS, and the message blamed the owner's Tailscale state for it. | The transport probes one thing: the open-source `tailscaled` unix socket, `/var/run/tailscaled.socket` on macOS. The Tailscale macOS application creates no socket. Both of its builds publish a loopback port and a shared secret instead: the standalone system-extension build writes `/Library/Tailscale/sameuserproof-<port>` with the secret inside the file, and the sandboxed App Store build puts the port and the secret in the name of a file in its group container. Probed directly before any code changed: the endpoint answered `200` with `BackendState: Running` and this node's DNS name and addresses. | The socket is still tried first, so a host running the daemon is unaffected. When it is absent, macOS discovery reads whichever proof form is present and speaks the LocalAPI over loopback with the secret as an HTTP Basic password, which is the form tailscaled's own clients use. Four tests cover both proof shapes, the unusable-proof case and the shared response parser. | pass: health now reports `available: true`, advertises `victors-macbook-air.<tailnet>.ts.net` and binds `100.90.226.66` alongside loopback. This unblocked the whole pairing chain on macOS: `H04` to `H11` and `K03` all pass, and `K03` printed exactly one QR advertising the tailnet address. |
| F37 | The approval gate failed open, and a live model walked straight through it. A run was told to delete a file and to get approval for that exact command first. `control__request_approval` returned `{"decision":"approve"}` without ever parking, the model ran the `rm`, and the run reported `completed`. The owner was never asked about a destructive action the goal had explicitly gated. | `DefaultPolicy` requires a human only when `risk` is exactly `"high"`; everything else falls through to `AutoAllow`. The tool schema declared `risk` as a bare `{"type":"string"}` with no enum and no description, so nothing told the model what the field accepted. The model filled it in the way the name invites, with a sentence: `"This permanently removes the file from disk."` That is not `"high"`, so the gate approved it. A schema is advisory and models violate it, so a gate that depends on the model choosing one exact token is not a gate. | The policy fails closed: only `low` and `medium` skip the owner, and `high` or anything unrecognised asks. The schema now names the three severities and says what `high` means. Four tests cover the two safe severities, `high`, the denylist, and the unrecognised values including the prose the live model actually sent. | pass: the same goal re-run against the fix sends `risk: "high"`, the run parks at `awaiting_approval` with one `await_user` entry, and the target file is still on disk. |
| F38 | On macOS the structured native tier answers with a Linux remedy. `ui_tree` against TextEdit returned `at_spi_unavailable` with `Enable it and install the stack with vadgr-cua install-deps (at-spi2-core plus the toolkit bridges)`, on a host whose Accessibility permission was granted and working and whose `doctor` reports all 33 tools. | The tier speaks AT-SPI, which only Linux has, and `resolve_backend()` only ever builds the AT-SPI backend; there is no macOS AX backend in the package. The README already says the native structured tier is Linux only, so the absence is a documented scope boundary and not a defect. The message was: it sent a macOS caller after `at-spi2-core`, a package the platform cannot have, so a tier that is deliberately not offered read as a broken install. | The remedy is chosen for the platform reporting it. Linux keeps the install line unchanged; macOS and Windows are pointed at the browser tier and the pointer tools. Two tests pin both branches. | pass for the message, shipped as cua `0.7.3`. **The capability gap itself stands and is the reason `OS-M` and `E01` record a pixel-tier sequence**: on macOS the editor task is driven by screenshots, clicks and typing rather than by an accessibility tree, the model needed several typing attempts to land the text, and the in-journal read-back is a screenshot rather than a structured text read. The oracle for those cells is therefore the independent AppleScript read of TextEdit's own document, which is the same shape of outside-both-tools oracle `OS-L` used. A macOS AX backend is a product decision and is not made here. |
