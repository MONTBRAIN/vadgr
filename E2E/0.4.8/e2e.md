...
| **overall** | **pass**, 1 partial | not run | **incomplete**: 43 of 45 cells | not run | **Windows**: A, B, C, D, E, F and H were driven to their stated oracles and pass; G is 3 of 4. The two open cells are `G3`, which needs a QR decoder independent of the encoder and none is installed on this host, and `H4`, which is partial for the same reason it is partial on WSL: no public CLI invocation can send a body the daemon rejects at validation. One product defect was found and fixed on this OS (`F7`), and one earlier finding was **retracted** (`F9`). **The count is 43 because two cells are genuinely not closed here, not because the pass was called done at a convenient number.** | every part of this runbook has been driven on WSL and every cell has a verdict. The one row that is not a clean pass is named above and it is not a WSL defect: `H4` has no public path to it. Three defects were found and fixed during the pass, each with a regression test seen red first |
