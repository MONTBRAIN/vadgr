...
> Private evidence PR #114, available to repository members.
>
...
>
> **WSL final targeted result: pass, 2026-08-28.** Three isolated roots against
> the rebuilt final code passed the affected CLI and Unix-permission boundaries
> under umasks `022` and `002` and with a permissive pre-existing root.
...
| # | Precondition and setup | Goal or action | Expected observable and oracle | Evidence boundary | Cleanup | Status |
|---|---|---|---|---|---|---|
| T1 | The daemon up with nothing set (default relays), behind the home NAT; a second network (phone hotspot or a namespace with no LAN route to the machine). **The dialing client must have Tailscale disconnected**, because the machine advertises its tailnet address as one of its direct addresses, so a client on the tailnet takes that path and the handshake proves the tailnet rather than traversal. Check it after the fact: if `selected_path` used the `100.64.0.0/10` address, the result is void. A relay path, or the public address, is the real thing | `vadgr pair` on the machine; read `node`, `relays` from the QR; from the second network, `$DIALER` dials the endpoint id and drives `GET /api/health` then a claim | the handshake completes; health answers `200`; the claim answers `200`. Record whether rendezvous was direct or relayed and the connect latency. Oracle: the daemon's `device paired` log line and the `device_peers` row | the dialer record, the daemon log for the window, the pairing report, the direct/relayed reading and latency | remove the device | pass: external macOS client; direct after relay rendezvous in 687 ms; health and claim `200`; daemon oracle and cleanup pass; macOS: pass, a WSL client on a second network with no route to the home LAN or the tailnet; `selected_path` `relay`, handshake 1256 ms, health `200` at public scope and the claim `200`; the daemon logged `transport=iroh` and bound the client's own endpoint id; Linux final head: pass from an external Apple Silicon Mac on a second network with Tailscale disconnected before dialing; public direct path after rendezvous, handshake 1121 ms, health and claim `200`, and the daemon log plus `device_peers` matched the client's handshake identity |
...
| F3 | A phone bound over the built-in transport; then `DELETE /api/devices/{id}` over loopback; another device row remains | the revoked phone dials again, reads health and attempts a protected read | deletion cascades the binding and token. Because adoption deliberately admits an unbound identity when the machine has any device row (`B8`), the handshake completes and public health answers with reduced fields; the protected read is `403 SOURCE_NOT_AUTHORIZED`. Oracle: the reduced health block, the protected-route refusal and the empty `device_peers` for the deleted id | the dialer record, the row read back | none | pass: corrected after adoption; final macOS and Linux both return reduced health `200`, devices `403`, and no deleted peer row |
...
| C: the claim binds | pass: Built-in bound exactly the handshake identity and transferred its one row on a second claim; loopback and Tailscale bound nothing | pass | pass | pass |
| H: health's scope | pass: loopback and bound tokenless Built-in received full reach; strangers with no token or junk token received only `name` and `available` | pass | pass | pass |
| S: the security surface | pass: S1-S6, including unminted pair refusal, no stranger reach disclosure, anti-eviction, window close and strict `node_key` rejection on Built-in and loopback | pass | pass: `S4` and `S5` are `B6` and `B5` read from the attacker side and cite that evidence | pass |
...
**Targeted closing result (2026-08-28, WSL): pass.** The installed artifact was
built from PR head `36567b7`, whose product source remains the final fix at
`ab5b6ad`, and had SHA-256 `ca23b3330712b98b374c7d3865277c8b65293a081eb524d0797da7e10b9dab60`.
Three concurrent isolated service-managed roots covered umasks `022` and `002`
and a deliberately permissive pre-existing root. Each root's installed CLI
reported exactly one default provider and one default model; the provider API
corroborated `gemini/gemini-3.7-flash`. Every service, state, credential, runs
and individual-run directory read `0700`; every service log, database family,
credential file and trajectory read `0600`. Each short run correctly ended
`NO_ACTION_TAKEN` after using only bookkeeping, and existed solely to make the
protected run and trajectory boundary observable. All owned daemons stopped,
their ports were free, and no unexpected product behaviour was observed.
