...
> `H4` is the same structural `partial` it is everywhere. The gate matches WSL
> and native Linux exactly, rust 272, api 445, cli 150, engine 122, with `fmt`
> and `clippy` at exit `0`. The rust count is 272 rather than 267 because this
> pass added five tests with its fix. `G4` paired a real handset over the
...
- `cargo test` in `rust/` -> **267 passed**, 1 ignored (Docker only)
...
| C8 | something already bound to the configured port | `vadgr start` | says the port is busy, walks up to a free one, and the port file names the port it actually took | CLI output, the listener list, the port file | stop, release the squatter | **pass**: with a squatter holding `127.0.0.1:8812`, the CLI printed `Port 8812 busy, using 8813`, started there, and the port file names `8813`. | **On macOS this cell failed and found `F15`**: the CLI printed `Port 8815 busy, using 8815`, naming the same port twice, and the daemon died on bind. Re-run against the fix in `00055b3` it prints `Port 8815 busy, using 8816`, the port file names 8816 and health answers there. The earlier passes observed the old search, so this cell is owed again on WSL, native Linux and native Windows.
