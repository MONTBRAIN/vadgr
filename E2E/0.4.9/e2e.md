...
defect was found on this operating system.** Two cells did not pass and neither
is Linux's: `I6` fails on the same default-branch layout Windows recorded, and
`F7` is `partial` because its subject cannot exist as the cell described it,
which is written up as `F17` and the cell corrected. **Three things Windows had
...
| F: runs and the watcher | pass | 8 of 9, `F7` partial | **pass**, 9 of 9 | not run | `F7` and `F8` both failed first and both were closed: `resume` now prints its row, and `F8`'s failure was an inherited ignore-Ctrl+C bit in the harness, not the product **Linux**: `F8` passes here exactly as written, `Detached. The run continues.` and exit `130`. `F7` is `partial` and the reason is the cell, not the product: its subject must be a failed run **that entered the loop**, which the cell text does not say, and a run that dies at the provider call writes no journal to grow. The resume half passes; `R2` evidences the journal-growth property on this OS. |
...
### F17 (open, and it is the runbook's): `F7` names a subject that cannot exist

`F7` asks for a run that failed on a missing model, and then asks that resuming
it grow the journal past its former last line to exactly two error lines. On
Linux those two clauses cannot both hold. A run given a model the provider does
not serve fails at the provider call, before the loop it would journal from, so
it writes no journal at all and there is nothing to grow. Every substitute built
the same way fails the same way, including a catalog model that refuses ordinary
generation.

The Windows record shows the way through without saying it in the cell: it was
"re-driven end to end on a failed run that had entered the loop". That is a
different subject from the one the cell's own precondition describes, and its
journal already carried a token-bearing line, which `F6`'s run never can.

The cell text is corrected above to require a subject that entered the loop.
Recorded rather than quietly re-driven, because the resume half passed on Linux
and the journal half was evidenced by `R2` on the same host, so the property is
covered on this OS and only the cell was wrong.
