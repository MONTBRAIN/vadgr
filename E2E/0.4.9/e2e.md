...
| I6 | `I5` done, the checkout two commits behind | `vadgr update` | it pulls fast forward only, rebuilds and reinstalls; the installed checkout's HEAD equals the origin's; **the binary hash changes when the pulled commits changed Rust source, and legitimately does not when they did not**, so record which kind of commits were pulled beside the hashes | the checkout's HEAD before and after, both binary hashes, and what the pulled commits touched | the container is removed | **fail on native Windows, with two independent defects, each reproduced.** **First, a freshly installed machine cannot run `vadgr update` at all**: exit `1` with `Could not run cargo: No such file or directory. An update from a checkout needs the Rust toolchain the installer set up.` The toolchain is there, at `/root/.cargo/bin/cargo`, but nothing puts it on `PATH`: `install.sh` runs rustup with `--no-modify-path`, sources `$HOME/.cargo/env` in its own process only, and `add_to_path` appends the install bin and nothing else. `grep -n cargo /root/.profile /root/.bashrc` returns nothing. **The error names the very thing the installer left unreachable.** **Second, with the toolchain reachable the build still fails**: `error: could not find Cargo.toml in /root/.vadgr/src`, because `vadgr update` follows `master` and `master` is the pre-cutover layout with the crate under `rust/`. The `0.4.9` command cannot build the tree its own default-branch update pulls, until `0.4.9` reaches `master`. **And the fast forward is not rolled back**: the checkout is left at the `0.4.8` source while the installed binary is still `0.4.9`, so `nothing was replaced. The installation you had is still the one on PATH` is true of the binary and false of the checkout. **The mechanism itself was then proved separately**, against a local mirror whose `master` is this branch, and marked as the substitute fixture it is: a docs-only range rebuilt to a **byte identical** binary, and a range touching `src/cli/commands/service.rs` produced a **different** one, which is what the cell asks for. **Two notes for the other hosts**: after any successful update the install root holds `vadgr` **and** `vadgr.previous`, so `A4`'s "no second file beside it" holds after a fresh install and stops holding after the first update; and both defects here are deterministic against today's `origin/master`, so the recorded `pass` on another operating system is worth re-reading. |
...
| # | Precondition and setup | Goal or action | Expected observable and oracle | Evidence boundary | Cleanup | Status |
|---|---|---|---|---|---|---|
| J1 | the platform state root absent or empty, checked and filed first: Linux and WSL `${XDG_STATE_HOME:-$HOME/.local/state}/vadgr`, macOS `~/Library/Application Support/vadgr/state`, Windows `%LOCALAPPDATA%\vadgr` | `env -u VADGR_STATE_HOME vadgr start` on a fresh port, then `env -u VADGR_STATE_HOME vadgr stop` | the daemon serves; `vadgr.db` and `credentials/` appear **under the platform root above**, and nothing appears under the working directory | the guard listing; the platform-root listing while serving; health; the working-directory listing | remove exactly the files the boundary listed, then show the root empty again | **blocked on native Windows, by the cell's own guard, and the guard is doing its job.** The platform state root is `%LOCALAPPDATA%\vadgr`, and it already holds two directories, `data` and `data\credentials`, dated `8/17`, before this pass began. The cell requires the root free of a real installation, so nothing there was touched and the listing is filed instead. **This is not a passive block**: the root holds none of `vadgr.db`, `runs/` or `credentials/` directly, and is not empty, so a real run today would refuse with `TargetOccupied`, which is a fixture problem rather than a product verdict. **What it uncovered is worth more than the cell**: a second state-root resolver exists. `default_state_root()` in `src/engine/provider/credentials.rs` falls back to `data_local_dir()` and yields `%LOCALAPPDATA%\vadgr\data\credentials`, while `config::state_root` returns `%LOCALAPPDATA%\vadgr`. The two disagree, and the second reads the real known folder rather than the `LOCALAPPDATA` variable, so it ignores an override the first honours. At `0.4.9` it looks unreachable from the daemon, since `Config::from_values` always sets `state_home`, and the directory timestamps predate this pass. **Unblocking J1 means deciding that tree is an artifact and moving it aside, which is the owner's call.** |
