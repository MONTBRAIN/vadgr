...
Status: B01-B06 passed on WSL and native Windows. B07-B10 remain blocked.
This check covers the candidate tooling, not an installer pass.
The implementation branch is `feat/trusted-candidate-bootstrap`.
Tested source: `1c0901dbd47dfa9f156bb67ac03235cea32254d5`.
Results: `candidate-bootstrap-results/`; the first failed boundary is retained.
...
|---|---|---|---|---|---|
| B01-WSL | WSL Python, no signing variables; record commit and script hashes | Invoke the public preflight command with a non-default workflow ref and dummy SHA | Nonzero refusal; no materialized source | Filesystem listing shows no output; API is not contacted | pass: macos-gate-pass.md; exact command, exit and filesystem oracle |
| B01-Windows | Native Windows Python; same setup as B01-WSL | Invoke the same preflight command through Windows Python | Same refusal and no materialized source | Native process exit and Windows filesystem listing | pass: macos-gate-pass.md; exact command, exit and filesystem oracle |
| B02-traversal-WSL | New traversal ZIP fixture on WSL; dummy authorization binds this commit | Invoke `extract-verified` for this fixture | Refusal names the unsafe archive property before extraction | No extraction directory or outside file exists; fixture SHA256 unchanged | pass: macos-gate-pass.md; exact command, exit and filesystem oracle |
...
| B03-WSL | A one-member authorization ZIP and a second ZIP with an extra member | Extract each through `scripts/candidate/record.py` | Valid record extracted exactly; extra member refused | Compare source/extracted SHA256 and absent second output | pass: macos-gate-pass.md; exact command, exit and filesystem oracle |
| B03-Windows | Native Windows paths, same two record cases | Run the record command with Windows Python | Same exact-copy success and extra-member refusal | Native SHA256 and process exit | pass: macos-gate-pass.md; exact command, exit and filesystem oracle |
| B04-Windows | Production PowerShell script and dummy non-default ref | Invoke signer with missing input and no vendor variables | Refuse before vendor access or file mutation | Native exit, script output, unchanged isolated directory | pass: macos-gate-pass.md; exact command, exit and filesystem oracle |
| B05-Windows | Production PowerShell script, attempt two, default-branch context | Invoke signer with missing input and no vendor variables | Refuse the rerun before reading a signing ledger | Native exit and absent ledger | pass: macos-gate-pass.md; exact command, exit and filesystem oracle |
| B06-Windows | Pinned JAR, Java compiler and dummy inputs only | Run the credential-safe launcher's missing-file self-test | Fixed safe output, no secret values, no log directory | Inspect raw captured output and all created files for dummy values | pass: exact pinned JAR and Temurin 17.0.20+8; no signing attempts |
...
|---|---|---|---|---|
| B01-B06 | pass: applicable WSL cells | pass: native boundaries and launcher | Not-Needed: Windows-only workflow; portable refusals covered on WSL | Not-Needed: Windows-only workflow; portable refusals covered on WSL |
| Activation B07-B10 | blocked: protected workflow prerequisites absent | blocked: protected workflow prerequisites absent | Not-Needed: no native Linux surface changed | Not-Needed: no native macOS surface changed |

Final signed installation and platform-trust cells belong to the product runbook.
These bootstrap checks do not award those results. The bootstrap pull-request gate is met by B01-B06. Protected activation and
signed installation remain blocked; they do not prevent review of the bootstrap.
