...
> **Status: macOS passed 86 of 86.** `I6` was the one cell owed, and it was driven after the release reached master; the earlier `85 of 86` below is the pre-release reading.
> Run on 2026-08-21 at `500d387` on macOS 26.5.2 build 25F84, arm64. **Native
> Windows is 86 of 86 as well**, with `I6` re-driven the same way against the
> released `0388fcf`. What follows is the pre-release reading, kept as it was
> written: `I6` was then the one cell that failed, for the same structural
> reason it failed on native Windows: `vadgr update` follows the released default branch by design, and
...
| A paid OpenAI account the owner can sign into (ChatGPT) | `O3`, `O5` | the owner says so; nothing is typed | one OAuth authorization | the connection is removed |
...
| # | Precondition and setup | Goal or action | Expected observable and oracle | Evidence boundary | Cleanup | Status |
|---|---|---|---|---|---|---|
| F1 | provider connected and default | `vadgr run "Take one screenshot, then reply done." --background` | exit `0`, the id printed; the run reaches `completed`; **the journal exists at that id with a screenshot tool's `in_flight` and `done` lines and a `response` line carrying `usage`** | CLI output; the run row from the API; the journal's phase counts | none | **pass on native Windows**: exit `0`, the id and watch hint printed, the run reached `completed`, and its journal holds 4 lines. `16760` input tokens, so the screenshot really went to the model, and the result is `done.` **pass on Linux**: a run reaches `completed` and the watcher prints the terminal row. |
| F2 | as F1; a fresh nonce in the task text | `vadgr run "Use your shell tool to run: echo vadgr-e2e-<nonce>. Then stop."` watched | `Run completed`, the results link, exit `0`; the journal is under the **state root's** `runs/`; **the nonce appears in exactly one `done` line** - the countable side effect | CLI output; the journal path and the nonce count | none | **pass on native Windows**: `Run completed (6s)`, the `See results:` line, exit `0`. The nonce appears **6 times** in the journal and the model returned the shell output `vadgr-e2e-7f3a91`, so a real shell ran it rather than the model describing what it would do. **pass on Linux**: the nonce the task was asked to emit appears where the cell requires it. |
| F3 | a run started and watched as F2 | from another terminal: `vadgr runs cancel <id>` - **the CLI, not a raw HTTP call** | cancel exits `0` and prints the row; the watcher says the run was cancelled and exits `0`; the row reads `cancelled` | both CLI outputs and exit codes; the run row | none | **pass on native Windows**: `vadgr runs cancel <id>` from a second terminal exited `0` and printed `Cancelled run <id>`; the watcher printed `Run cancelled (8s)` and **exited `0`**; the row reads `cancelled`. This is the improvement the release owns, and the shipped Python CLI printed nothing at all here. **The first attempt was a harness fault, recorded rather than hidden**: the task finished in 14s before the cancel landed, and the CLI correctly refused with `Run is already finished`, exit `1`. **pass on Linux**: a background run detaches and is followed from a second command. |
| F4 | as F1 | `vadgr run "<task>" --background --json` | stdout parses whole, with no hint on it; the row says `queued` | the output through a strict parser | none | **pass on native Windows**: stdout parses whole under a strict parser, the row reads `queued`, and `Watch it with` is absent from the stream. The `0.4.8` fix survives the cutover. **pass on Linux**: cancel lands and the row reads `cancelled`. |
| F5 | as F1; the default model's id known from E2 | `vadgr run "<task>" --provider gemini --model <that id> --json` watched | the first stdout block parses as the run row naming that provider and model; the watch ends `Run completed`, exit `0`; the API row's `provider` and `model` equal the flags | CLI output; the parsed block; the API row | none | **pass on native Windows**: the first stdout block parses on its own with **nothing after it**, carries the explicit `gemini` and `gemini-3.7-flash`, and the run completed with the shell output. **The first attempt was a harness fault**: the task said only `Reply with one word.`, which takes no action, and this engine fails such a turn with `NO_ACTION_TAKEN`. The `0.4.8` runbook recorded the same trap; the task was mine to choose and I chose badly. **pass on Linux**. **The first attempt was discarded as the cell text's fault, not the product's**: the task `Reply with one word.` gives the agent nothing to do and failed `NO_ACTION_TAKEN`; re-driven with a task that acts, it passes. |
| F6 | as F1 | `vadgr run "Reply with one word." --provider gemini --model vadgr-e2e-no-such-model` watched | the daemon accepts the run (creation does not read the catalog); the first provider call fails; the watcher reports the failure and **exits `1`**; the row reads **`failed`**; the journal carries an `error` line | CLI output and exit code; the row; the journal's error line | F7 consumes this run | **pass on native Windows**: creation accepted the run, then it failed with `model `gemini/vadgr-e2e-no-such-model` is not connected`, exit `1`, and the row reads `failed` with that error in `outputs`. **One wording defect recorded rather than smoothed**: the message is prefixed `run recovery failed:` on a run that was never recovered. It is a first attempt, and the prefix sends a reader looking for a recovery that did not happen. **pass on Linux**: a model the provider does not serve fails the run at recovery and the error names the provider's own refusal. |
| F7 | a failed run **that had entered the loop**, so it has a journal to grow: a task that takes no action serves, and `F6`'s run does **not**, because a run that dies at the provider call writes no journal at all | `vadgr runs resume <id>` - **the CLI, positive path** | resume is accepted and prints the row; the row passes through `running`; **the journal grows past its former last line** and its resumed segment carries the recovered context; it fails again the same way, and the journal then holds **exactly two** records, the second written by the resumed segment - one per attempt, the count that proves the resume really ran | both CLI outputs; the journal's line count before and after, and the two error lines | none | **pass on native Windows**, against the corrected cell, and re-read rather than re-run: the observation already answers the oracle as `F17` rewrote it. Journal **1 record to 2**, the second written by the resumed segment and carrying the recovered context, `input_tokens` 7834 against 7800, the difference being the replayed prior turn. Resume accepted, exit `0`, the row printed with `Status: running`, and the status sequence `failed, running, failed`. **The defect this cell found was fixed in `6a22164`**: `vadgr runs resume` printed one line naming an id the owner had just typed and no row. The detail block now lives in one printer that both `get` and `resume` call, and the row is **read back** rather than echoed from the acceptance, because the supervisor writes `running` before it spawns and a run that dies at once would otherwise be reported as running to an owner who cannot act on it. **And this pass is one of the two `F17` names.** The subject was substituted, because `F6`'s run writes no journal, and that was explained in this status instead of in the cell. The cell therefore went on describing a subject that cannot serve it, and native Linux met it as a genuine block. Linux handled it correctly by recording `partial` rather than substituting quietly, which is what exposed the defect. The old oracle asked for **two error lines** and no host has ever seen one here: the records are `response` phases and the error lives on the run row, so all three passes read that clause loosely. |
| F8 | a run started and watched as F2, mid-flight | send SIGINT to the watching CLI | the watcher prints "Detached. The run continues." and **exits `130`**; the run was **not** cancelled: the API row later reads `completed` | the watcher's output and exit code; the row after | none | **pass on native Windows.** All three oracles hold: the watcher printed `Detached. The run continues.` with the two follow-up lines, exited **`130`**, and the run was **not** cancelled, polling `running` from `t+0s` to `t+40s` and `completed` at `t+45s`. **This cell was recorded as a failure first, and that verdict was mine, not the product's.** The interrupt never reached the watcher because Windows carries an ignore-Ctrl+C bit in a process's parameters that **every child inherits**, and the drivers set it in the process that then spawned the product, so the product started deaf. Proved by A and B trials rather than argued: same binary, same command, same delivery, ignore bit set gives ignored and still running after 20 seconds, ignore bit cleared gives exit `0`. The handler was in `src/cli/stream.rs:142-149` the whole time. **pass on Linux**: the watcher printed `Detached. The run continues.` and exited **`130`**, and the run was not cancelled: the API row later read `completed`. |
| F9 | F1-F8 have left `completed`, `failed` and `cancelled` rows | `vadgr runs list --status failed`, and `curl "$API/api/runs?status=failed"` | both list exactly the failed runs and no other, and equal each other; repeat for `--status completed` and compare counts | both outputs, side by side | none | **pass on native Windows**, once `F3` had left a `cancelled` row: the database held `completed 5`, `failed 3`, `cancelled 1`, and `vadgr runs list --status <s>` matched `GET /api/runs?status=<s>` on every one, exit `0` each time. **pass on Linux**: CLI and API agree exactly, `failed` 4 and 4, `completed` 4 and 4, `cancelled` 1 and 1. |
...
| # | Precondition and setup | Goal or action | Expected observable and oracle | Evidence boundary | Cleanup | Status |
|---|---|---|---|---|---|---|
| R1 | a watched run mid-flight, on a task shaped "call your time tool's sleep for 30 seconds, then run: echo vadgr-e2e-<nonce>, then stop" (the released runtime's `time` tool ships `sleep`, capped at 60 seconds); the kill lands during the wait | `kill -9` the daemon pid | the watcher prints "Lost the run stream. The run continues in the background." and exits `0` - **the errored-stream line, corrected after two operating systems recorded the same thing**: `src/cli/stream.rs` carries a sibling line for a clean close, and a hard kill never produces it - the deliberate no-verdict outcome, observed on purpose; the run row in the database file still reads an active status, read with `sqlite3` directly, since no daemon is alive to ask | the watcher output and exit code; the row read from the file | R2 | **pass on native Windows**: the daemon was killed `7.55s` into a 30 second sleep, with the sleep confirmed `in_flight` first. The watcher exited `0`, and the database read directly with no daemon alive holds `status running` and `completed_at None`. **Two things recorded rather than smoothed.** The watcher printed `Lost the run stream. The run continues in the background.`, not the line this cell quotes; both exist in `src/cli/stream.rs`, one for a clean close and one for an errored stream, and a hard kill produces the errored branch, so **the runbook quotes the sibling line**. And the first attempt was discarded because the kill landed 1.8s after the sleep finished, so its precondition was never met. **pass on Linux**: the daemon was killed with the sleep confirmed in flight, the journal at 5 lines. The watcher exited **`0`** and the row read straight from `vadgr.db` with no daemon alive holds `running` and `completed_at None`. **It reproduces the Windows observation exactly**: the watcher printed `Lost the run stream. The run continues in the background.`, the errored-stream sibling, not the line this cell quotes. Two operating systems now agree, so the runbook quotes the wrong one of the two lines in `src/cli/stream.rs`. **On WSL this cell is `partial`, corrected on re-reading its own artifact rather than left claiming a pass.** The filed record carries the run id, the row read from the database file with no daemon alive, and the pid gone after `SIGKILL`, so the second oracle held. **It carries no watcher output and no exit code**, which is the other half, and its own header says the kill landed while the first model turn was still open, with `journal lines at the kill: 0`, so the sleep this cell asks to interrupt was never in flight. Native Windows discarded an attempt for that same miss and Linux confirmed the sleep in flight at 5 journal lines; both filed the watcher line and `WATCHER_EXIT=0`. It is not re-driven here because the computer-use runtime this host used has since been removed and the cell's task needs its `time` tool. The cell is proven on two native operating systems and the defect it exposed, `F18`, was found by them. |
| R2 | R1's state root, daemon dead, one active run in it | start the daemon | the log carries the recovery scan line with `resumed=1`; the run reaches a terminal state; **the journal grew past its pre-kill end**, the resumed segment is marked as such, and **the nonce appears in exactly one `done` line across the whole journal** - interrupted plus recovered is still once | the recovery log line; the journal's pre-kill and final line counts; the nonce count | stop | **pass on native Windows** on its evidenced oracles: the boot log reads `run recovery scan complete resumed=1`, the run reached `completed`, the journal grew from 2 lines to 9, and the nonce appears in **exactly one `done` line**, which is the recovered-not-replayed proof. **A gap worth naming**: the journal carries no marker for the resumed segment at all. `src/engine/journal.rs` writes six phases and none of them says resumed, and the resumed segment restarts `iteration` at `0`, so the file holds two `response` records both labelled iteration 0, separable only by a 51 second timestamp gap. The resume is marked outside the journal, in the daemon log and in a `run_resumed` socket event. **Incidental, and it belongs to the computer-use runtime on Windows**: the first shell call after recovery failed with `[WinError 2] The system cannot find the file specified`, and the model retried with `shell_mode` and succeeded. **pass on Linux**: the boot log reads `run recovery scan complete resumed=1 parked=0 failed=0`, the run reached `completed`, the journal grew from **5 lines to 12**, and the nonce appears in **exactly one `done` line**, which is the recovered-not-replayed proof. **The Windows gap is confirmed on a second OS**: the journal carries no marker for the resumed segment at all, zero lines in the file mention a resume. |
| R3 | provider connected and default | `vadgr run "Use your ask_user tool to ask the owner whether to continue, and wait for the answer." --background`, then watch it from a second terminal | the row reaches **`awaiting_approval`**; the watcher prints the waiting-for-approval line; the socket carries an `awaiting` frame. **Disposition, stated rather than silent**: this release ships no reply surface for a parked run - the engine's own source says so at `src/engine/control/hitl.rs` - so the shipped exits are cancel and boot re-park, and this cell proves the park is reachable, visible on every surface, and safe. The reply surface belongs to the release that ships the conversation surface | the row; the watcher line; the captured `awaiting` frame | R4 consumes this run | **pass on native Windows**: the row reached `awaiting_approval`, the watcher showed `Waiting for your approval: Would you like to continue?`, and the sockets carried `{agent_started, awaiting, run_started}` on the CLI route and `{started, tool_call, paused}` on the phone route. **The cell is not executable exactly as written**: the CLI ships no verb that attaches a watcher to an existing run, so the watched form was used, which is the same `stream::follow` path. **A trap for the other hosts**: the waiting line renders through a spinner that is off when the stream is not a terminal, so a redirected capture records nothing and reads as a false negative. It was read from the console screen buffer instead. **pass on Linux**: the row reached `awaiting_approval`, the sockets carried `{agent_started, awaiting, run_started}` on the CLI route and `{started, tool_call, paused}` on the phone route, frame for frame what Windows recorded. **The spinner trap is confirmed**: a redirected capture holds only the started line, while the same command under a pty shows `Waiting for your approval: Do you want to continue?`. |
| R4 | R3's run parked | restart the daemon, then `vadgr runs cancel <id>` | the recovery scan line says `parked=1` and the row still reads `awaiting_approval` after boot; the cancel then lands: the row reads `cancelled`, and **the daemon stays healthy** - health answers `200` and the log holds no panic | the recovery log line; the row before and after the cancel; health; a grep of the log for panics | stop | **pass on native Windows**: `vadgr restart` exit `0`, the boot log reads `resumed=0 parked=1`, and the row still read `awaiting_approval` afterwards, so a parked run is not resumed by a restart. `vadgr runs cancel` exit `0` then left the row `cancelled` with a `completed_at`, health answered `200`, and the log holds zero panics and zero `ERROR` lines. **pass on Linux**: `vadgr restart` exit `0`, the boot log reads `resumed=0 parked=1`, and the row still read `awaiting_approval` afterwards, so a parked run is not resumed by a restart. `vadgr runs cancel` exit `0` left the row `cancelled` with a `completed_at`, health answered `200`, and every daemon log from this group holds zero panics and zero `ERROR` lines. |
...
**`0.4.9` reached `master` as `0388fcf` on 21 August 2026, and `I6` was
re-driven on native Linux, then on macOS, then on native Windows.** It passes on
all three: the layout failure that made it the one open cell on three operating
systems is gone, and `F14` is closed by observation on each of them. Reaching
the comparison for the first time also exposed `F21`, that this cell's
binary-hash clause cannot be exercised by the sequence `I5` leaves behind.

| # | Precondition and setup | Goal or action | Expected observable and oracle | Evidence boundary | Cleanup | Status |
|---|---|---|---|---|---|---|
| I1 | a container with a shell, git and curl, **no toolchain and no vadgr** | assert both absences | `cargo` and `vadgr` are not found | the two failed lookups | the container is removed | **pass on native Windows**, in `debian:bookworm-slim` with only `git`, `curl` and `ca-certificates` added: `cargo`, `rustc`, `vadgr`, `cc` and `gcc` all resolve to nothing, exit `1` each, and `/root/.vadgr` does not exist. **pass on Linux**, in `debian:bookworm-slim` under podman with only `git`, `curl` and `ca-certificates` added: `cargo`, `rustc`, `vadgr`, `cc`, `gcc` and `rustup` all resolve to nothing, exit `1` each, and `/root/.vadgr` does not exist. |
| I2 | as I1 | run `install.sh` against the commit under test | it installs the toolchain, builds, and puts both binaries in the install root; **`git rev-parse HEAD` inside the container equals the PR head recorded in `A1`** - the invalidated pass drifted here | the transcript, the install root listing, both head hashes side by side | as I1 | **pass on native Windows**: the script was fetched inside the container by commit and its `sha256` equals `git show <head>:install.sh`, so the installer under test is the commit's own. Exit `0`. The checkout's `HEAD` inside the container equals the host head exactly. The install root holds **one** executable, `vadgr`, 22760088 bytes. Cost recorded: rustup 1.5G, cargo 335M, the install 748M, container 3.19GB, build `1m 02s`. **Observation, not a defect**: `install.sh` printed `Detected OS: wsl` inside a Docker Desktop container, because that kernel carries `microsoft` in `/proc/version`. The `linux` and `wsl` branches are identical, so nothing followed from it. **pass on Linux**: the script was fetched inside the container **by commit** and its `sha256` is `873ec749cd656a172ef5620bb115842d3e06cf9b9e2644ddd0e2f393672a6e8c`, equal to `git show <head>:install.sh`, so the installer under test is the commit's own, and the same blob Windows drove. Exit `0`, build `3m 28s`. The checkout's `HEAD` inside the container equals the host head `5b753f88` exactly. The install root holds **one** executable, `vadgr`, 22837416 bytes. **A host fault worth recording, since it is not the product's**: the first container run failed at `Installed a C toolchain but cc is still not on PATH`, because this machine's clock was hours behind real time and `apt` refused Debian's Release files as not valid yet. With the clock corrected the container was rebuilt untouched and the installer ran clean. |
| I3 | as I2 | `vadgr --version`, then `vadgr health` | the version matches; health exits `3` because nothing is started, which is the correct answer | CLI output and both exit codes | as I1 | **pass on native Windows**: the installed `vadgr --version` prints `vadgr 0.4.9`, matching `Cargo.toml`, and `vadgr health` with no daemon exits `3` with `API is not running at http://127.0.0.1:8000. Start it with: vadgr start`. **pass on Linux**: the installed `vadgr --version` prints `vadgr 0.4.9`, matching `Cargo.toml`, and `vadgr health` with no daemon exits **`3`** with `API is not running at http://127.0.0.1:8000. Start it with: vadgr start`. |
| I4 | as I3; the clone at a head not behind the released default branch | `vadgr update --check` | exit `0` with "vadgr is up to date." | CLI output and exit code | as I1 | **pass on native Windows**: `vadgr update --check` printed `vadgr is up to date.`, exit `0`, and the oracle agrees: `git rev-list --count HEAD..origin/master` is `0`. **pass on Linux**: `vadgr update --check` printed `vadgr is up to date.`, exit `0`, and the oracle agrees: `git rev-list --count HEAD..origin/master` is `0`. |
| I5 | as I4, then `git -C ~/.vadgr/src reset --hard origin/master~2` | `vadgr update --check` | exit `0`; "2 commit(s) available"; the Cargo.lock line printed **when and only when** the range touches `Cargo.lock` - record which it was | CLI output; `git log --oneline` of the range; whether the range touches `Cargo.lock` | as I1 | **pass on native Windows**: with the checkout reset two commits behind, `vadgr update --check` printed `2 commit(s) available. Run 'vadgr update' to apply them.`, exit `0`. **The `Cargo.lock` line was absent and that is the correct half of the rule**: the range touches workflows, docs and scripts only, and `git diff --name-only HEAD..origin/master -- Cargo.lock` is empty, as is the same query for `*.rs` and `Cargo.toml`. So this run recorded the negative side of "when and only when". **pass on Linux**: reset two commits behind, `vadgr update --check` printed `2 commit(s) available. Run 'vadgr update' to apply them.`, exit `0`. **This run also recorded the negative side of "when and only when"**: the range is `61a6a39` and `a830700`, touching `.github/workflows/secret-scan.yml`, `E2E/README.md`, `E2E/TEMPLATE.md`, `scripts/check_evidence.py` and its test only, `git diff --name-only HEAD..origin/master -- Cargo.lock` is empty, and the `Cargo.lock` line was absent. **Re-driven after the release, this host now holds the other half of "when and only when" too**: with `master` at `0388fcf`, the two-commit range is the cutover itself plus `eda8027`, it touches `Cargo.lock` and 98 Rust files, and `vadgr update --check` printed `2 commit(s) available.` **followed by `dependencies change: Cargo.lock`**. Absent when the range does not touch it, present when it does, both sides now evidenced on one operating system. **Native Windows holds both sides too, re-driven the same day**: against `master` at `0388fcf` the two-commit range is the cutover plus `eda8027`, touching `Cargo.lock`, `Cargo.toml` and 97 Rust files, and `vadgr update --check` printed `2 commit(s) available. Run 'vadgr update' to apply them.` followed by `dependencies change: Cargo.lock`, exit `0`. The negative side is the reading this host recorded before the release, on a range that touched workflows, docs and scripts only, so the two Windows records are the two halves of the rule. |
| I6 | `I5` done, the checkout two commits behind | `vadgr update` | it pulls fast forward only, rebuilds and reinstalls; the installed checkout's HEAD equals the origin's; **the binary hash changes when the pulled commits changed Rust source, and legitimately does not when they did not**, so record which kind of commits were pulled beside the hashes | the checkout's HEAD before and after, both binary hashes, and what the pulled commits touched | the container is removed | **pass on native Windows now, re-driven against the released `master`; the full Windows reading is at the end of this cell, beside Linux and macOS.** What follows immediately is the pre-release Windows record, kept because it is where both of this cell's defects were found. **fail on native Windows before the release, on one of its two defects. The other is fixed and the cell was re-run to prove it.** **Defect one, fixed in `a83ff1c` and verified end to end**: a freshly installed machine could not run `vadgr update` at all, because nothing put the toolchain on `PATH`. `install.sh` ran rustup with `--no-modify-path`, sourced `$HOME/.cargo/env` in its own process only, and wrote just the install bin into the rc file, so the error named the very thing the installer had left unreachable. Re-driven from nothing after the fix, against the **committed blob** `873ec749...a6e8c` rather than the working copy: the container began with no `cargo`, `rustc`, `vadgr`, `cc`, `gcc` or `rustup`, exit `1` each. The installer printed `Added the Rust toolchain to PATH in .bashrc, which vadgr update needs`, a new login shell then resolved `cargo 1.97.1` and `vadgr`, and `vadgr update` exited `0` with `Updated 1 binary/binaries.` and no toolchain error. **The counterfactual is what makes it conclusive**: the same binary in a shell that does not read the rc file reproduces the old `Could not run cargo` failure, exit `1`. The negative branch holds too: a machine that already had cargo got the `PATH` line and **no** toolchain line, and a second install added no duplicate. **Defect two, still failing and not fixable here**: `vadgr update` follows `master`, and `master` still carries the pre-cutover layout, so with the checkout reset behind it the build fails with `error: could not find Cargo.toml in /root/.vadgr/src`, exit `1`, and the fast forward is not rolled back. Cargo itself produced that error, so the two failures are now cleanly separable. It resolves when `0.4.9` reaches `master`. **One residual gap recorded rather than glossed**: the new line is written when *this installer* set the toolchain up, not when cargo will be reachable in the next shell. A user whose cargo exists only inside the installing process is still uncovered; a normal rustup user has rustup's own line and is unaffected. **pass on Linux, re-driven after `0.4.9` reached the default branch**, which is the condition this cell was owed and the only thing that was ever wrong with it. Driven from nothing in a fresh `debian:bookworm-slim` container under podman: no `cargo`, `rustc`, `vadgr`, `cc`, `gcc` or `rustup`, exit `1` each. Installed from the released `master` commit `0388fcf`, exit `0`, checkout `HEAD` equal to it. Reset two commits behind, `vadgr update` then **pulled fast forward only, rebuilt and reinstalled, exit `0`**, printing `Updated 1 binary/binaries.`, and the installed checkout's `HEAD` equals the origin's. `vadgr --version` still reads `vadgr 0.4.9` and `vadgr health` still exits `3`. **Defect one stays fixed**: the toolchain was on `PATH` in a login shell and no toolchain error appeared. **Defect two is gone**: `master` now carries `Cargo.toml` at its root, so the build the cutover broke succeeds. **The hash clause is a separate matter and is recorded rather than claimed**: the pulled range touched 98 Rust files and `Cargo.lock`, yet the binary hash is unchanged, `efb8fb2b...09870` either side. That is correct here and the cell cannot show otherwise. The binary was replaced, its mtime is the update's, and it hashes the same because `install.sh` had already built it from `0388fcf` and `git reset --hard origin/master~2` moves the **checkout** and never the binary, so the update rebuilds the source the binary already came from. See `F21`. **pass on macOS too**, driven the same way after the release: a `debian:bookworm-slim` container with no `cargo`, `rustc`, `vadgr`, `cc`, `gcc` or `rustup`, installed from `0388fcf` and updated from two commits behind. `vadgr update` exited `0` with `Updated 1 binary/binaries.`, the installed checkout's `HEAD` equals the origin's, and `vadgr --version` still reads `0.4.9` with `vadgr health` at exit `3`. **The hash clause behaved exactly as `F21` describes, on a second operating system**: the pulled range changed 97 rust files, the binary's mtime moved from `1787281526` to `1787281621` so it was genuinely replaced, and its `sha256` was `9feac0c5...3e3a` on both sides. macOS is **86 of 86**. **pass on native Windows too, re-driven after the release on the host that found both of this cell's defects.** Driven from nothing in a `debian:bookworm-slim` container under Docker Desktop `29.2.1`, with only `git`, `curl` and `ca-certificates` added: no `cargo`, `rustc`, `vadgr`, `cc`, `gcc` or `rustup`, exit `1` each, and `/root/.vadgr` absent. Installed from the released `0388fcf` against the **committed blob**, whose `sha256` inside the container is `873ec749...a6e8c` and equals `git show 0388fcf:install.sh` on the host, so the CRLF working copy is not what ran. Exit `0`, build `1m 03s`, and the container's `HEAD` equals `0388fcf`. **The `a83ff1c` fix holds here**: a new login shell, `bash -lc` so the rc file is read, resolved `cargo 1.98.0` and `vadgr`. Reset two commits behind, `vadgr update` then **pulled fast forward only, `Updating 78e32e5..0388fcf` then `Fast-forward`, rebuilt in `36.57s`, reinstalled and exited `0`** with `Updated 1 binary/binaries.`, and the checkout's `HEAD` afterwards equals `origin/master`. `vadgr --version` still reads `vadgr 0.4.9`, `vadgr health` still exits `3`, and neither `Could not run cargo` nor `could not find Cargo.toml in /root/.vadgr/src` appears anywhere in the transcript. **`F21` reproduces here, and this host is what settles it**: the pulled range changed 97 Rust files and `Cargo.lock`, the binary's mtime moved from `1787282453` to `1787282550` so it was genuinely replaced, and its `sha256` is `efb8fb2b...09870` either side, **byte for byte the hash native Linux recorded**, from a different operating system, a different container runtime and a separate image pull. Two hosts, one binary, which is the determinism `F21` argues from, observed rather than argued. **A correction to a number and not to a verdict**: the Linux record says 98 Rust files in this range, while this host and macOS both count 97, and `git diff --name-only 78e32e5..0388fcf -- '*.rs'` on the host returns 97. Nothing in any verdict turns on it. Windows is **86 of 86**. |
...
**Native Linux was then driven end to end, against head `5b753f88` on Ubuntu
26.04, and every part was run.** All 86 cells were driven one command at a time
against a single set of binaries, hashed before the pass and unchanged
throughout, so no cell names an artifact that stopped existing. **No product
defect was found on this operating system.** One cell does not pass and it is not Linux's:
`I6` fails on the same default-branch layout Windows recorded. `F7` was recorded
`partial` first, because its subject cannot exist as the cell described it, and
was then driven on the subject both earlier hosts had silently substituted, where
it passes. The cell is corrected and `F17` records it. **Three things Windows had
recorded as single-host observations were independently reproduced here** rather
than taken on trust: the watcher's errored-stream line, the journal's silence
about a resumed segment, and the spinner that hides the waiting line when the
stream is not a terminal. The first of those is now a runbook fix, `F18`.

macOS is `not run` for these rows, and a green gate on it would say the suites
pass there and nothing about whether the product works.
...
| I: the installer and update | **pass, and worth re-reading** | **pass, 6 of 6**, re-driven after the release | **pass, 6 of 6**, re-driven after the release | **pass, 6 of 6**, re-driven after the release | **The two results are different depths of one cell, not a disagreement about the product.** The WSL row carries the word `pass` and no evidence. On Windows an update that pulls nothing behaves the same way and passes: the checkout is ahead of `master`, the pull is a no-op, and the rebuild builds the `0.4.9` tree. The layout failure appears only when the update **actually pulls**, which was forced here by resetting the checkout behind `master`. Both `I6` defects are deterministic against today's `master`, so the WSL pass most likely took the no-op path the cell does not intend | in a container with no toolchain and no product **Linux reaches the same verdict as Windows, in `debian:bookworm-slim` under podman**: the `a83ff1c` toolchain fix is confirmed here including its counterfactual, and `vadgr update` still cannot build a `master` that carries the pre-cutover layout. **Those two readings are the pre-release ones. After `0388fcf` landed the three native hosts agree the other way**: `master` carries `Cargo.toml` at its root, `I6` was re-driven from nothing on Linux, macOS and Windows, and it passes on each. |
...
| **overall** | 85 of 86, `R1` partial | **86 of 86**, the owed cell driven after the release | **pass, 86 of 86**, the owed cell driven after the release | **86 of 86**, the owed cell driven after the release | the weakest part actually driven on this OS, which is every part. **WSL**: every part driven, and `R1` is `partial` rather than `pass` because its filed artifact records the row and the killed pid but no watcher output and no exit code, on a run whose sleep was never in flight. The cell is proven natively on Linux and Windows. **Windows**: every part driven, and the repeatability check run as three concurrent passes with their own port, database and daemon. The one cell not passing is `I6`, and only half of it: its installer defect is fixed and re-driven from nothing, while `vadgr update` still cannot build a `master` that carries the pre-cutover layout, which resolves when this release lands there. **It landed as `0388fcf`, and `I6` was re-driven from nothing on this host against it: the pull is fast forward, the rebuild succeeds, `vadgr update` exits `0`, and native Windows is 86 of 86.** **Six defects were found and fixed on this operating system**, each with a test that fails without it and each verified by re-running the cell that found it. **Three findings were retracted as harness faults rather than left standing**: `vadgr start` never hung, the provider never rejected our identity, and `F8`'s watcher always had its handler. Each retraction is recorded where the claim was made **Linux**: every part driven on a native host. The two cells not passing are `I6`, owed by the default branch and identical to the Windows verdict, and `F7`, which is `partial` because its subject cannot exist on this host as the cell describes it. **No product defect was found on Linux this pass**, and three Windows observations were independently confirmed here rather than taken on trust. |
...
**Linux owes none of them, and the check was run rather than assumed.** The
native Linux pass was driven end to end against head `5b753f88`, and each of the
seven commits above was confirmed an ancestor of it with `git merge-base
--is-ancestor`, so every Linux row in the table above names code that still
exists. The cells the fixes were most likely to turn red were driven and
recorded: `CU2` reads `false` for a disabled module on both surfaces, `B5`,
`B6`, `B7` and `B10` each show a refusal naming what it found, `C1`, `C5` and
`C13` bind rather than probe, `F7` prints its row on resume, and `I2` and `I6`
carry the toolchain line. **WSL and macOS still owe theirs.**

**What the Linux pass changed, and who owes a re-run for it: nobody.** The Linux
commit touches `E2E/0.4.9/e2e.md` and nothing else, so no cell on any operating
system names code that stopped existing, and rule 4 is not engaged. Two cells
were corrected in text, and in both cases the correction is the runbook catching
up with what the earlier hosts already observed rather than a new expectation:

| corrected | what a host that already passed it should do |
|---|---|
| `R1`, quoted line | **nothing on Windows**, whose filed output already reads `Lost the run stream. The run continues in the background.`, which is what the cell now says. **WSL should read its own record rather than re-run**: `R1.txt` there captures the run, the pid and the row, and does not capture the watcher's line at all, so that clause was never evidenced on WSL. Recording it next pass is enough |
| `F7`, subject and oracle | **nothing on either**. Both already drove the corrected subject, a failed run that had entered the loop, and both recorded the corrected oracle, one journal record growing to two. The cell now says what they did |

**Neither correction can turn an earlier pass red**, which is the test for
whether a runbook edit creates debt. `F19` is a finding about the product and
changes no cell, so it creates none either.
...
### F14 (fixed by the release, and verified): update follows a default branch with the old layout
...
**Closed on 21 August 2026, and closed by observation rather than by argument.**
`0.4.9` reached `master` as `0388fcf` and `master` now carries `Cargo.toml` at
its root. `I6` was re-driven on native Linux from nothing, in a fresh container
with no toolchain and no product, installing from the released commit and then
updating from two commits behind: the pull is fast forward, the rebuild
succeeds, `vadgr update` exits `0` with `Updated 1 binary/binaries.`, and the
checkout's `HEAD` equals the origin's. This was the last cell owed on any
operating system for `0.4.9`.

**Confirmed on the two hosts that also failed it.** macOS re-drove it the same
way and passes. **Native Windows, which found this finding, re-drove it from
nothing against `0388fcf`**: the pull is `Updating 78e32e5..0388fcf` then
`Fast-forward`, the rebuild takes `36.57s`, `vadgr update` exits `0` with
`Updated 1 binary/binaries.`, the checkout's `HEAD` afterwards equals
`origin/master`, and the error this finding is named for,
`error: could not find Cargo.toml in /root/.vadgr/src`, appears nowhere in the
transcript. Fixed on the host that raised it, which is what closes it.

### F21 (open, and it is the runbook's): `I6`'s binary-hash clause cannot be exercised by `I6`'s own procedure

`I6` asks that the binary hash **change when the pulled commits changed Rust
source, and legitimately not when they did not**, and asks the host to record
which kind was pulled. Driven after the release, on a range that touches 98 Rust
files and `Cargo.lock`, the hash did not change: `efb8fb2b...09870` either side.

That is not the product misbehaving, and the cell cannot show that it is. The
binary was genuinely replaced, its mtime is the update's. It hashes the same
because the sequence the cell prescribes leaves the binary **ahead of the
checkout, never behind it**. `install.sh` builds from the tip, `I5` then runs
`git reset --hard origin/master~2`, and that moves the **checkout** and nothing
else. So `vadgr update` pulls the checkout back to the tip and rebuilds the very
source the installed binary was already built from. A deterministic build then
returns the same bytes, which is exactly what was observed, twice: two
independent containers, one built from the branch head and one from the released
`master`, produced the identical hash.

**The clause is therefore unfalsifiable as written on every operating system**,
not just this one. To exercise the change side, the binary must be built at the
old commit, which means installing at `origin/master~2` and only then updating.
That is a different setup from the one `I5` leaves behind, and `I6` inherits
`I5`'s state by design.

It could not have been noticed before now. Native Windows and native Linux both
failed this cell at the build, on `F14`, and never reached the comparison, and
the release is what let anyone reach it. Recorded rather than fixed, because
choosing the replacement setup is a decision about what `I5` should leave behind
and belongs with whoever owns the part, not with the host that happened to get
there first.

**Confirmed independently on macOS.** The same procedure on a `debian:bookworm-slim`
container under colima, installing from `0388fcf` and updating from two commits
behind, pulled a range touching 97 rust files and `Cargo.lock`. The binary's
mtime moved from `1787281526` to `1787281621`, so it was genuinely rebuilt and
replaced, and its `sha256` was `9feac0c5...3e3a` on both sides. Two operating
systems, two container images, two different hashes from each other, and the
same result within each host: the clause cannot fail while `I5` leaves the
binary ahead of the checkout. That it reproduces on a second platform is what
makes it a property of the procedure rather than of one host's build.

**Native Windows is the third, and it turns the argument into an observation.**
The same procedure in a `debian:bookworm-slim` container under Docker Desktop
`29.2.1`, installing from `0388fcf` and updating from two commits behind, pulled
a range touching `Cargo.lock` and 97 Rust files. The binary's mtime moved from
`1787282453` to `1787282550`, so it was rebuilt and replaced, and its `sha256`
is `efb8fb2b...09870` on both sides. **That is byte for byte the hash native
Linux recorded**, on a different operating system, a different container runtime
and a separate image pull. The determinism this finding rests on no longer has
to be inferred from two containers on one host: two hosts produced one binary.

**One correction to a number, not to the finding.** The Linux record above says
the range touches 98 Rust files. Windows and macOS both count 97, and the host's
own `git diff --name-only 78e32e5..0388fcf -- '*.rs'` returns 97. The Linux
record is left as its host wrote it, because nothing in the finding or in any
verdict turns on the count.

### F15 (open, and it needs both repos): a typed pairing never learns the machine's name

The Machines list named this machine `santiago-casa-1.tail32...` rather than
`Santiago-Casa`. The same handset, paired by scanning, showed the name.

`POST /api/auth/pair` returns `machine_name`, and the QR deep link carries it.
A handset that types the code never sees that response, and `POST
/api/auth/claim` answers with the token and the device id only. `GET
/api/health`, which the row is otherwise drawn from, carries no name either, so
the app falls back to the address it was given. The app documents this as its
behaviour rather than stumbling into it.

**It is open because the daemon half cannot ship alone.** The app names the
machine from the payload it built before the claim, and its claim result type
carries two fields; adding `machine_name` to the claim response now would add a
field no client reads, which is the exact dead field this release deleted from
`GET /api/settings/computer-use`. The two halves are one paired iteration, and
they are planned in the next mobile minor. Found by `H1` on real glass.

### F16 (open, and it is mobile's): a sheet that watched a run live keeps what the socket gave it

The run sheet was open and watching when the run ended. In that state it showed
**no produced text at all** and a duration of `15s`. Closing it and tapping the
same run again showed `I see "echo one" and "echo two" in a terminal window.`
and `20s`, both matching the row the daemon serves.

When the stream closes, the sheet keeps the last thing the socket gave it rather
than re-reading `GET /api/runs/<id>`. So a finished run is drawn with no result,
and its clock stops at the last frame rather than at completion: `20.4s` of run
was drawn as `15s`.

**Nothing here is the daemon's.** The row the re-open reads is the row it served
the whole time, and the same sheet renders correctly whenever it is opened after
the run rather than during it. Found by `H4` on real glass, and it is the
sharper cause of the completion-state defect already recorded against the minor
that puts runs on the phone.

### F17 (fixed in this commit, and it is the runbook's): `F7` named a subject that cannot serve it, and two hosts silently used another

`F7`'s precondition said `F6`'s run, and `F6`'s run is given a model the
provider does not serve. That run fails at the provider call, before the loop it
would journal from, so it writes **no journal at all**. The cell then asks that
resuming it grow the journal past its former last line. On that subject the two
clauses cannot both hold, on any operating system.

**Both earlier hosts substituted a different subject without saying so in the
cell.** WSL's filed artifact resumes a run that failed `NO_ACTION_TAKEN` on the
task `Reply with exactly the word: ready. Use no tools.`, whose journal held one
line before the resume and two after. Native Windows recorded the same shape,
one line to two with the new line carrying `input_tokens`, and described the
cell only as "re-driven end to end on a failed run that had entered the loop".
A run given a missing model can never carry `input_tokens`, so neither pass used
the subject the cell named, and the cell kept describing one that cannot work.

Linux hit this as a genuine block, recorded the cell as `partial` rather than
substituting quietly, and then drove it on the same subject the other two had
used, where it passes: journal 1 line to 2, resume accepted, row printed,
`Status: running`, failing again the same way.

Two corrections above, and the second was found by the first. The precondition
now names a failed run that entered the loop and says why `F6`'s cannot serve.
The oracle used to ask for "exactly two error lines", and **no host has ever
observed an error line in this journal**: the records are `response` phases and
the error lives on the run row, so all three passes were reading the clause
loosely. It now asks for two records, the second written by the resumed segment.

The lesson is the one the runbook already states and this cell defeated:
substituting a subject without amending the cell leaves the next host to
rediscover that the cell is impossible, and to doubt its own machine first.

### F18 (fixed in this commit, and it is the runbook's): `R1` quoted the wrong one of two sibling lines

`src/cli/stream.rs` carries two closing lines, one for a stream that ends
cleanly and one for a stream that errors. `kill -9` produces the errored branch,
so the watcher prints `Lost the run stream. The run continues in the
background.` and the cell quoted the other one.

Windows recorded this as an observation on its own pass. Linux reproduced it
exactly, on a different kernel and a different process supervisor, which is what
turns one host's curiosity into a defect in the runbook. The quoted text is
corrected above. Nothing in the product changed: both lines exist deliberately
and the cell's real oracle, exit `0` and an active row read from the file, held
on both operating systems.

### F19 (open, low, and it is a reporting gap rather than a fault): a missing computer-use runtime reaches the user as `NO_ACTION_TAKEN`

A daemon started without `VADGR_CUA_BIN` on a machine where `vadgr-cua` is not
on `PATH` registers no tools. `vadgr computer-use status` still answers
`Computer use: enabled`, because the setting is enabled and the setting is what
it reports, and a run then fails in about two seconds with `NO_ACTION_TAKEN`,
which describes the model's behaviour rather than its cause.

The daemon log does name the cause precisely, `MCP server was dropped
server="computer-use" reason="MCP server failed: vadgr-cua was not found"`, so
nothing is hidden from a reader who knows to look. What is missing is that the
run's own error, the only thing a CLI user sees, cannot distinguish a runtime
that never loaded from a model that declined to act.

Found while standing up an isolated root for the `R` group, where the
