...
\author{
\IEEEauthorblockN{Victor Santiago Monta\~{n}o Diaz}
...
% ============================================================
\begin{abstract}
Current AI computer use agents treat every click as a first-time click. No matter how many times an agent clicks the same button, the movement takes the same amount of time. Humans, by contrast, get faster with practice. We present a spatiotemporal motor memory cache that gives autonomous GUI agents the ability to develop ``muscle memory.'' The system combines eight mechanisms: R-Tree spatial indexing, Power Law of Practice speed-up, Fitts's Law movement timing, WindMouse trajectory adaptation, temporal decay, EMA position smoothing, action sequence prediction, and cross-session persistence. A three-layer resolution hierarchy (model-provided hints, accessibility API detection, and percentage-bucketed coordinates) ensures that \emph{every} mouse interaction builds cache entries, regardless of application or platform. Testing on real desktop applications across four operating systems shows cache hits growing across sessions and across independent agent instances, with movement durations dropping to 30\% of initial values for frequently-used targets. To our knowledge, this is the first system to apply cognitive science motor learning models to the execution layer of AI GUI agents.
\end{abstract}
...
\textbf{Ghost-Cursor}~\cite{ghostcursor} implements static Fitts's Law simulation for browser testing with B\'{e}zier curves. Every movement to the same target takes the same amount of time. There is no learning and no cross-session persistence.
...
Fig.~\ref{fig:architecture} illustrates the system's data flow. The motor memory cache sits between the decision layer (LLM determining \emph{what} to click) and the execution layer (platform-specific cursor control). The LLM does not need to know the cache exists. It does not change what gets clicked, only how fast and how directly the cursor reaches the target.
...
The cache algorithm is platform-agnostic (pure Python, stdlib \texttt{sqlite3}). Platform-specific operations (foreground window queries, cursor control, accessibility API access) are handled by backend modules:
...
\textbf{Works with any LLM.} The motor memory cache operates in the execution layer. It works with Claude, GPT-4, open-source models, or any other decision framework. It does not change \emph{what} the agent clicks, only how fast the cursor gets there.

\textbf{Trajectories converge naturally.} As hit count grows, the WindMouse parameter adaptation produces trajectories that approach straight lines with smooth velocity profiles. This resembles the minimum-jerk trajectory~\cite{soukoreff2004} seen in expert human pointing. We did not program this directly; it emerges from the parameter adaptation.

\textbf{Shared memory across agents.} Because the cache is stored in SQLite on disk, multiple independent agent instances share the same motor memory. Agent~B moves faster to targets that Agent~A has already used. No explicit knowledge transfer is needed.

\textbf{Living system.} The parameters reported here ($\alpha = 0.4$, $\beta = 0.3$, $\tau = 7$~days, EMA $\alpha = 0.3$, spatial radius = 50~px) are all configurable. As more data is collected from real usage, these values can be tuned or made adaptive.
...
We have presented the first system to apply cognitive science motor learning models (Power Law of Practice, Fitts's Law, Ebbinghaus temporal decay) to the execution layer of autonomous GUI agents. The spatiotemporal motor memory cache, with its three-layer resolution hierarchy and eight mechanisms, lets any computer use agent develop muscle memory that persists across sessions and transfers between agent instances.
