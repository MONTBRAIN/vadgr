...
| B: credential storage | 4 platforms x 8 assertions | 32 | 8 | 24 | 0 |
...
| B: credential storage and migration | not run | not run | not run | **pass** | the eight cases exist per platform as `BL`, `BM`, `BW` and `BQ`. 8 of 8 `BQ` cells pass, including the drvfs root WSL alone can produce. `BL`, `BM` and `BW` need their own hosts |
