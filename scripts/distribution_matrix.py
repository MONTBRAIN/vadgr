...
def remaining_builds(architecture):
    selected = "windows-" + {"x64": "x86_64", "arm64": "aarch64"}[architecture]
    return [row for row in matrix() if row["target"] != selected]
...
def verify_payload(root, target, pins_path):
    ...
    require(manifest.get("target") == spec["rust_target"]
            and manifest.get("cua_version") == pins["cua"]
...
def main():
    ...
    try:
        if args.command == "matrix":
            print(json.dumps({"include": remaining_builds(args.except_windows) if args.except_windows else matrix()}, separators=(",", ":")))
    ...
    except (Refused, OSError, KeyError, TypeError, json.JSONDecodeError) as error:
