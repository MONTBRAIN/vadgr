...
> **Status: run on all four supported operating systems, and complete on three
> of the four.** **Native Windows is now `partial`, and the reason is evidence
> rather than behaviour**: its artifacts were filed on 2026-08-20, two days
> after the pass, and nine of its 45 cells had left nothing on disk. Those nine
> keep their readings and are marked `not run`, because a cell is `pass` only
> when the observation and the artifact both exist. The Windows boundary is
> `e2e_evidence/vadgr-0.4.8/20260819-windows` and the cells are named in the
> per-OS section.
>
> macOS was the last one owed and ran on 2026-08-19 at `00055b3` on
...
|---|---|---|---|---|---|---|
| A1 | `$E2E_HOME/bin` first on `PATH` | `command -v vadgr` | resolves inside `$E2E_HOME/bin`, never to a system or Python `vadgr` | the resolved path and its `sha256` | none | **pass** on `e3799b1`: `command -v vadgr` resolved to `/tmp/vadgr-048-e2e/bin/vadgr`, whose `sha256` matches the release build of the head under test. **On native Windows this is `not run`**: the reading stands as the Windows pass recorded it, and the artifact was not captured during the pass, so this reading cannot be re-derived. The binaries are still in that host's test root and `20260819-windows/host.txt` carries their hashes, read two days later, which is not the hash line this cell asserts. |
| A2 | as A1 | `vadgr --version` | prints `vadgr 0.4.8`, matching the crate manifest and `api/config.py` | the printed line | none | **pass**: `vadgr 0.4.8`, agreeing with `rust/Cargo.toml` and `api/config.py`, which a unit test keeps in step. **On native Windows this is `not run`**: the reading stands as the Windows pass recorded it, and the artifact was not captured during the pass, so this reading cannot be re-derived. |
| A3 | as A1 | `vadgr --help` | lists every shipped verb, and names none of `registry`, `agent`, `workflow`, `project`, `forge` | the help text | none | **pass**: all fifteen verbs listed. `registry`, `agent`, `workflow`, `project` and `forge` appear zero times. **On native Windows this is `not run`**: the reading stands as the Windows pass recorded it, and the artifact was not captured during the pass, so this reading cannot be re-derived. |

## Part B: one family of names, and an address resolved in the stated order

| # | Precondition and setup | Goal or action | Expected observable and oracle | Evidence boundary | Cleanup | Status |
|---|---|---|---|---|---|---|
| B1 | Rust daemon listening on `8811`; no port file | `VADGR_PORT=8811 vadgr health` | exit `0`, the daemon's own `/api/health` body agrees with the printed fields | CLI output, `curl` read-back | none | **pass on `d1b3f2f`**, re-run after `F7` changed the platform field. Exit `0`, and every printed field equals the `/api/health` body read with `curl`. **The first attempt at this re-run was a harness fault, recorded rather than hidden**: it used the home `C1` had just written a port file into, so the CLI correctly followed that file to the Python daemon while the oracle read the Rust one. The cell's own precondition says no port file, and it passes against a clean home. **On native Windows this is `not run`**: the reading stands as the Windows pass recorded it, and neither the CLI output nor the `curl` read-back was captured during the pass, so this reading cannot be re-derived. |
| B2 | daemon on `8811`; `VADGR_PORT` set to a dead port | `VADGR_PORT=8899 VADGR_API_URL=http://127.0.0.1:8811 vadgr health` | exit `0`: the URL wins over the port | CLI output | none | **pass**: exit `0` with `VADGR_PORT` naming a dead port, so the URL wins. **On native Windows this is `not run`**: the reading stands as the Windows pass recorded it, and the artifact was not captured during the pass, so this reading cannot be re-derived. |
| B3 | as B2 | `VADGR_API_URL=http://127.0.0.1:8899 vadgr --api-url http://127.0.0.1:8811 health` | exit `0`: the flag wins over the environment | CLI output | none | **pass**: exit `0` with `VADGR_API_URL` naming a dead port, so the flag wins. **On native Windows this is `not run`**: the reading stands as the Windows pass recorded it, and the artifact was not captured during the pass, so this reading cannot be re-derived. |
...
| B5 | a port file naming a port whose pid is dead | `vadgr health` | the stale file is removed and the default is used; exit `3` with nothing listening | the directory listing before and after | none | **pass**: a port file naming pid `999999` was ignored, **both files were removed**, and the CLI fell to the default `8000` and exited `3`. **On native Windows this is `not run`**: the reading stands as the Windows pass recorded it, and neither the CLI output nor the directory listing either side of the run was captured during the pass, so this reading cannot be re-derived. The listing is the whole oracle of this cell. |
...
| G3 | G2's output | rebuild the deep link from the printed `Machine`, `Address` and `Pairing code`, encode it at the shipped settings, and compare with the rendered symbol | the two renders are identical, so what is on the screen encodes exactly the link the phone needs | the two renders and their comparison | none | **pass**: `rqrr`, a decoder independent of the encoder under test, read the symbol **as printed** and recovered `vadgr://pair?host=santiago-casa-1.tail323b9e.ts.net&port=8811&token=N36R-GRHC&name=Santiago-Casa`, which is exactly the link rebuilt from the fields printed beside it. Version 5 at error correction level `Low`, as the probe chose. **On native Windows this is `not run`**: the reading stands as the Windows pass recorded it, including the link it says `rqrr` recovered, and **the decoder's own output was not captured during the pass**. The rendered symbols survive in the Windows boundary, and a symbol is not a decode, so this reading cannot be re-derived. |
...
| H5 | Rust daemon | `vadgr health > file` and `vadgr runs list > file` | the files contain no escape sequence; the same commands on a terminal are coloured | the two files, and a byte scan for `0x1b` | none | **pass**: `health`, `runs list` and `status` redirected to files contain zero escape bytes. **On native Windows this is `not run`**: the reading stands as the Windows pass recorded it, and only one of the three redirections was captured, `H/H5-runs-list-forced.txt`, which does hold zero `0x1b` bytes. Two thirds of the reading cannot be re-derived, so the cell is not claimed on this OS. |
...
| A: the binary is the built head | **pass**, 3 of 3 | **pass**, 3 of 3 | **not run**, 0 of 3 filed | **pass**, 3 of 3 | the installed command resolves inside the test root and its `sha256` is the release build of the head under test. **Native Windows filed nothing for this part.** All three readings stand as the Windows pass recorded them, no artifact supports any of them, and the column reads `not run` because a reading nobody can re-derive is not a pass. |
| B: address resolution and the rename | **pass**, 6 of 6 | **pass**, 6 of 6 | **pass**, 2 of 6 filed; `B1`, `B2`, `B3` and `B5` `not run` | **pass**, 6 of 6 | includes the two cells that matter most for the rename: a live port file beating the environment, and the old `FORGE_*` names reaching nothing. **Native Windows filed `B4` and `B6` only.** `B1`, `B2`, `B3` and `B5` left no artifact, so their Windows readings stand and cannot be re-derived. `B5`'s oracle is a directory listing either side of the run, and neither side was captured. |
| C: the service group, Python daemon | **pass**, 8 of 8 | **pass**, 8 of 8 | **pass**, 8 of 8 | **pass**, 8 of 8 | the Rust CLI starts, supervises, follows and stops the **Python** daemon, which is the strangler seam this release must not break. **Native Windows filed all eight**, including both halves of `C8`: the squatter killing the daemon and then the walk to `18813`. |
| D: read-only commands, Rust daemon | **pass**, 6 of 6 | **pass**, 6 of 6 | **pass**, 6 of 6 | **pass**, 6 of 6 | `health`, `providers`, `computer-use status` and `status` are byte for byte identical to the shipped Python CLI on the same daemon. Two defects were fixed to get there, F1 and F3. **Native Windows filed all six**, and the byte-for-byte claim is readable there from two `health` renders, one per CLI, that compare equal. |
| E: provider onboarding | **pass**, 7 of 7 | **pass**, 7 of 7 | **pass**, 7 of 7 | **pass**, 7 of 7 | two real credentials, a live 28 model catalog and a live 10 model catalog, one bounded readiness call each, and the secret absent from the database, WAL, SHM and evidence. **Windows**: re-driven from a fresh database against a daemon on `18816`, because an earlier drive on this OS left no re-readable captures and corroboration is not evidence. `E1` named `GEMINI_API_KEY` rather than its value and reported `Ready: Google Gemini, Gemini 3.7 Flash`; `E3` was proved live by the daemon's own `POST /api/providers/gemini/catalog-refresh status=200` in `153 ms`; `E6` refused with `the default provider cannot be disconnected` exactly as it does on WSL; `E6b` connected Anthropic's 10 model catalog, printed `Default remains: Google Gemini / gemini-2.5-flash-lite`, and its credential record went from two files to one on logout. **The secret-absence oracle was run with the daemon stopped**, because a first attempt could not read the database, WAL, SHM or daemon log while it held them open and would have reported a clean scan of the files that mattered least: 22 files, zero plaintext occurrences of either key, and the credential store's ACL is owner plus `OWNER RIGHTS` and `SYSTEM` only. **Native Windows filed all seven**, plus the daemon log carrying the `E3` refresh at `200` in `153 ms`. The credential directory listing `E6b` names is not filed, and the credential files themselves are excluded by rule. |
| F: runs and the watcher | **pass**, 6 of 6 | **pass**, 6 of 6 | **pass**, 6 of 6 | **pass**, 6 of 6 | `F3` is the release's own improvement, and it is proved by driving the shipped Python CLI against the same cancellation, where it printed nothing and hung. **Native Windows filed all six**, plus the ten run journals. `F3` was driven twice there and both cancellations are filed. The Python CLI comparison was driven on WSL and there is no Windows capture of it. |
| G: pair and the QR | **pass**, 4 of 4 | **pass**, 4 of 4 | **pass**, 3 of 4 filed; `G3` `not run` | **pass**, 4 of 4 | `G3` decodes the printed symbol with an independent decoder; `G4` is a real handset, and the daemon recorded the claim at `200` and the device row. **On native Windows the decode itself was not captured**, only the rendered symbols, so `G3` reads `not run` there. `G4` holds, and it holds on the daemon's own log: `POST /api/auth/claim` answered `410` on an expired code and then `200`, with `GET /api/devices` behind it. |
| H: negatives and exit codes | **pass**, 4 of 5, 1 partial | **pass**, 4 of 5, 1 partial | **pass**, 3 of 5, `H4` partial, `H5` `not run` | **pass**, 4 of 5, `H4` partial | `H4` is partial: no public CLI invocation can send a malformed body, so the `422` parser is covered by its unit test and the wire shape by a live probe. **On native Windows `H5` reads `not run`**: one of its three redirections was captured, and it does hold zero escape bytes. The Windows `H4` body is a different malformed request from the one quoted above and the same list shape. |
| **overall** | **pass**, 1 partial, re-run on `0a7fb17` | **pass**, 1 partial | **partial**, 35 of 45 pass, 1 partial, 9 `not run` | **pass**, 44 of 45, 1 partial | **Windows**: every part was driven to its stated oracles and read back, and the pass reported 44 of 45. **That figure was a reading of the terminal, not of a filed artifact**, and it does not survive the boundary: see the sentence at the end of this cell. `G3` used this runbook's own `harness/qr-decode`, which builds and runs unchanged here: `rqrr` read the symbol **as printed** and recovered `vadgr://pair?host=santiago-casa.tail323b9e.ts.net&port=18811&token=6Z6E-6DQH&name=Santiago-Casa`, the link rebuilt from the fields beside it, at version 5 and ecc level Low. **That decode was never written to a file**, so `G3` is `not run` here. `H4` is partial and **earned rather than inherited**: the `422` was captured live on this OS, its body is the list shape the parser reads, and its message names the field, `missing field \`task\``. One product defect was found and fixed on this OS (`F7`), and one earlier finding was **retracted** (`F9`). **The Windows evidence boundary was filed on 2026-08-20, two days after the pass, and it is partial.** It is `e2e_evidence/vadgr-0.4.8/20260819-windows` in the docs repository, 119 files. Nine cells left nothing on disk and were downgraded to `not run` with their readings kept: `A1`, `A2`, `A3`, `B1`, `B2`, `B3`, `B5`, `G3` and `H5`. The pass also recorded an exit code exactly once, in `F/F3d-exit-code.txt`, so every other Windows status line naming an exit code names something the boundary does not carry. No gate output was captured on this host either, so the `partial, api only` row above rests on the same prose it always did. | every part of this runbook has been driven on WSL and every cell has a verdict. The one row that is not a clean pass is named above and it is not a WSL defect: `H4` has no public path to it. Three defects were found and fixed during the WSL pass, each with a regression test seen red first. **Four cells were then re-run on `d1b3f2f`** after the native Windows pass fixed two more: `B1`, `C1` and `D1` for `F7`, which changed the platform field on every OS, and `F4` for `F11`, whose earlier WSL pass is retracted because a lenient oracle hid the defect. **`C8` was then re-run again on `0a7fb17`** after macOS found `F15` in the port search, and the flip that defect needs was reproduced on this host before the cell was re-driven |

**Native Windows, driven 2026-08-18, and its evidence filed 2026-08-20.**
Windows 11 Pro build 26200, native rather than WSL, with the cells `F15`
invalidated re-run the same evening. The boundary is
`e2e_evidence/vadgr-0.4.8/20260819-windows` and **it is partial**, which is why
the Windows column above no longer reads a clean pass on every part.

The pass read each cell correctly and filed nothing. The captures stayed in
`%TEMP%\vadgr-048-e2e` on the host that produced them, and the runbook carried
the readings alone for two days. Filing them showed which readings had nothing
under them. **Nine cells left no artifact and are now `not run` with their
readings kept**: `A1`, `A2`, `A3`, `B1`, `B2`, `B3`, `B5`, `G3` and `H5`. Part
`A` filed nothing at all. `G3` kept its rendered symbols and lost its decode,
which is the half that carries the claim. `H5` kept one of the three
redirections it names.

Three more things a reader should know before trusting a Windows status line.
The pass captured stdout and stderr per cell and **recorded an exit code
exactly once**, so the exit codes in these status lines are readings rather
than artifacts. Several filed cells kept their own output and lost the second
oracle beside it: `C1`'s pid file, `C4`'s `tail`, `E6b`'s credential listing,
`G1`'s empty device list, and `H1`'s wall-clock duration, which is the whole of
finding `F10` on this OS. And **the `vadgr` binary that ran the first pass was
overwritten** by the rebuild for the `F15` re-run, so no hash in the boundary
identifies the CLI that produced the earlier captures.

None of this is a defect in the product, and none of it changes a reading. It
is the cost of filing evidence after a pass instead of during it, recorded here
