...
re-driven on native Linux the same day.** It passes: the layout failure that
made it the one open cell on three operating systems is gone, and `F14` is
closed by observation. Reaching the comparison for the first time also exposed
`F21`, that this cell's binary-hash clause cannot be exercised by the sequence
`I5` leaves behind.
