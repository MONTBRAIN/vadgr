...
|---|---|---|---|---|
| H: protected owner boundaries | partial development: F01 QR/Built-in and typed-code/Tailscale completed; signed W02 remains owed | partial development: physical saved-name QR/Built-in pairing completed; protected M02 and signed subject remain owed | not run: the final Linux candidate and physical phone are unavailable | Not-Needed: WSL has no native GUI or protected installer prompt |
...
| L: native Linux cells | Not-Needed: native-Linux-only cells | Not-Needed: native-Linux-only cells | not run: x86_64 Wayland development observations are recorded below; current head lacks the private CUA payload and reviewed package inputs, and the final Linux candidate is unavailable | Not-Needed: native-Linux-only cells |
...
| Linux x86_64/aarch64 X11/Wayland | L01, L02, F01, L03 through L06, O1, O2, C1 | not run: historical unsigned x86_64 Wayland development probes exist, but the current head cannot produce a package without the private CUA payload and reviewed package inputs; the signed candidate, phone, offline snapshot, aarch64 and X11 remain unavailable |
